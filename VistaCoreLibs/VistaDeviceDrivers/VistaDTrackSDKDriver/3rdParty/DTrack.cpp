...
)
{
	...
	if(remote_host != NULL && remote_port != 0){
		...
		if(d_remote_ip == 0){  // resolving of hostname was not possible
			free(d_udpbuf);
			udp_exit(d_udpsock);
			d_udpsock = NULL;
			return;
		...
		if(!strncmp(s, "gl ", 3)){
			...
			for(i=0; i<n; i++){                           // get data of hands
				...
				for(j=0; j<act_hand[id].nfinger; j++){     // get data of fingers
					...
					act_hand[id].finger[j].m_fAnglephalanx[0] = farr[2];
					act_hand[id].finger[j].lengthphalanx[1] = farr[3];
					act_hand[id].finger[j].m_fAnglephalanx[1] = farr[4];
...
static void* udp_init(unsigned short port)
{
   ...
   long nsock = long(sock);
	if(bind(OS_SOCKET(nsock), (struct sockaddr *) &name, sizeof(name)) < 0){
		udp_exit(sock);
		return NULL;
	}
...
static int udp_receive(void* sock, void *buffer, int maxlen, int tout_us)
{
	...
	while(1){
		...
		nbytes = recv(OS_SOCKET(nsock), (char *)buffer, maxlen, 0);
...
static int udp_send(void* sock, void* buffer, int len, unsigned int ipaddr, unsigned short port, int tout_us)
{
	...
	nbytes = sendto(OS_SOCKET(nsock), (char* )buffer, len, 0, (struct sockaddr* )&addr,
