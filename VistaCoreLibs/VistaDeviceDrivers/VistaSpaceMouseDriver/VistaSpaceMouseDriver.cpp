...
bool VistaSpaceMouseDriver::DoSensorUpdate(VistaType::microtime dTs)
{
	...
	case 'd':
		{
			...
			m_qOri = VistaQuaternion(
				VistaEulerAngles(
							(Vista::Pi/180)*(fCoords[3]),
							(Vista::Pi/180)*(fCoords[4]),
							(Vista::Pi/180)*(fCoords[5])) );
			break;
		}
	...
	VistaSpaceMouseMeasures::sSpaceMouseMeasure *m = (VistaSpaceMouseMeasures::sSpaceMouseMeasure*)&(*pM).m_vecMeasures[0];
