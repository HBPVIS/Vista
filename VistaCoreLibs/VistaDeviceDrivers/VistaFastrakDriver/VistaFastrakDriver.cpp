	...
	while(n < 3*GetCommandSet()->CmdGetActiveStations())
	{
				...
				&nSensor, &fPx, &fPy, &fPz, &fOw, &fOx, &fOy, &fOz, &nButtonFlag, &nTimeStamp, &nFrameCount, &nStylus, &nDistortionLevel )) == 13 )
		{
			...
			if(nStylus == 0) // simple body
			{
				...
				VistaFastrakMeasures::sFastrakButtonSample *pMeasure = (VistaFastrakMeasures::sFastrakButtonSample*)&(*pM).m_vecMeasures[0];
		...
		if(bRet)
		{
			...
			for(int n=0; n < sizeof(long)*8; ++n)
			{
				if( nStations & (1<<n) )
				{
					...
					pSensor->SetTypeHint( GetFactory()->GetTypeFor( "WAND" ) );
