...
namespace
{
	...
	public:
			...
			unsigned int nIndex ) const
		{
			...
			VistaDirectXGamepadSensorMeasures::VistaDirectXGamepadSensorMeasure *m = (VistaDirectXGamepadSensorMeasures::VistaDirectXGamepadSensorMeasure*)&(*pMeasure).m_vecMeasures[0];
	...
	public:
		...
		{
			VistaDirectXGamepadSensorMeasures::VistaDirectXGamepadSensorMeasure *m = (VistaDirectXGamepadSensorMeasures::VistaDirectXGamepadSensorMeasure*)&(*pM).m_vecMeasures[0];
			...
			case TR_POS:
				{
					return VistaVector3D(float(m->m_pDirectXJoystate2.lX), float(m->m_pDirectXJoystate2.lY), float(m->m_pDirectXJoystate2.lZ));
				}
			case TR_ROT:
				{
					return VistaVector3D(float(m->m_pDirectXJoystate2.lRx), float(m->m_pDirectXJoystate2.lRy), float(m->m_pDirectXJoystate2.lRz));
				}
			case TR_VEL:
				{
					return VistaVector3D(float(m->m_pDirectXJoystate2.lVX), float(m->m_pDirectXJoystate2.lVY), float(m->m_pDirectXJoystate2.lVZ));
				}
			case TR_AVEL:
				{
					return VistaVector3D(float(m->m_pDirectXJoystate2.lVRx), float(m->m_pDirectXJoystate2.lVRy), float(m->m_pDirectXJoystate2.lVRz));
				}
			case TR_ACCEL:
				{
					return VistaVector3D(float(m->m_pDirectXJoystate2.lAX), float(m->m_pDirectXJoystate2.lAY), float(m->m_pDirectXJoystate2.lAZ));
				}
			case TR_AACCEL:
				{
					return VistaVector3D(float(m->m_pDirectXJoystate2.lARx), float(m->m_pDirectXJoystate2.lARy), float(m->m_pDirectXJoystate2.lARz));
				}
			case TR_FORCE:
				{
					return VistaVector3D(float(m->m_pDirectXJoystate2.lFX), float(m->m_pDirectXJoystate2.lFY), float(m->m_pDirectXJoystate2.lFZ));
				}
			case TR_TORQUE:
				{
					return VistaVector3D(float(m->m_pDirectXJoystate2.lFRx), float(m->m_pDirectXJoystate2.lFRy), float(m->m_pDirectXJoystate2.lFRz));
				}
			case TR_POVAXIS:
				{
	...
	class VistaDirectXGamepadDIJOYSTATE2Get : public IVistaMeasureTranscode::TTranscodeValueGet<VistaDirectXGamepadSensorMeasures::VISTADIRECTXJOYSTATE2>
	{
	public:
		VistaDirectXGamepadDIJOYSTATE2Get()
			: IVistaMeasureTranscode::TTranscodeValueGet<VistaDirectXGamepadSensorMeasures::VISTADIRECTXJOYSTATE2>("DIJOYSTATE",
			VistaDirectXGamepadMeasureTranscode::GetTypeString(),
		...
		virtual VistaDirectXGamepadSensorMeasures::VISTADIRECTXJOYSTATE2 GetValue(const VistaSensorMeasure *pM)    const
		{
			VistaDirectXGamepadSensorMeasures::VistaDirectXGamepadSensorMeasure *m = (VistaDirectXGamepadSensorMeasures::VistaDirectXGamepadSensorMeasure*)&(*pM).m_vecMeasures[0];
			return m->m_pDirectXJoystate2;
		}

		virtual bool GetValue(const VistaSensorMeasure *pM, VistaDirectXGamepadSensorMeasures::VISTADIRECTXJOYSTATE2 &oOut) const
		{
			if(!pM)
				return false;
	...
	public:
		...
		virtual VistaQuaternion GetValue(const VistaSensorMeasure *pM)    const
		{
			...
			{
				VistaVector3D vRes = VistaDirectXGamepadAxisVectorTranscode::GetPOV2Vec(m->m_pDirectXJoystate2.rgdwPOV);
	...
	public:
		...
		virtual std::string GetTranscoderName() const { return "VistaDirectXGamepadTranscodeFactory"; }
...
#if VISTADIRECTXGAMEPADTRANSCODER_EXPORTS
