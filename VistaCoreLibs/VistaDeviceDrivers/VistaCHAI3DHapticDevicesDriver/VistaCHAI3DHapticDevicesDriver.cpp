...
VistaCHAI3DHapticDevicesDriver::~VistaCHAI3DHapticDevicesDriver()
{
	...
	// close haptic device
    if(m_pHapticDevicePrivate->m_pHapticDevice)m_pHapticDevicePrivate->m_pHapticDevice->close();
...
bool VistaCHAI3DHapticDevicesDriver::Connect()
{
	...
	if(!m_pHapticDeviceHandler)
	{
		std::cerr << "[CHAI3DHapticDevicesDriver]: Could not create CHAI3D HapticDeviceHandler" << std::endl;
		return false;
	}
	...
	for ( int i = 0; i < (int)numHapticDevices; i++ )
	{
		...
		if ((m_pHapticDevicePrivate->m_strDeviceString).compare(a_deviceSpecification.m_modelName) == 0)
		{
	...
	if(!(m_pHapticDevicePrivate->m_pHapticDevice))
	{
		...
		if(!(m_pHapticDevicePrivate->m_pHapticDevice))
		{
			std::cerr << "[CHAI3DHapticDevicesDriver]: Also can't get a handle to the first haptic device" << std::endl;
			return false;
		}
		else{
			cHapticDeviceInfo a_deviceSpecification;
			...
			std::cerr << "[CHAI3DHapticDevicesDriver]: Took the first device that came along: " << m_pHapticDevicePrivate->m_strDeviceString << std::endl;
		}
	}
   ...
   // open connection to haptic device (0 indicates success).
	...
	{
		std::cerr << "[CHAI3DHapticDevicesDriver]: Can't open connection to the haptic device" << std::endl;
		return false;
	}
	...
	//we assume here that each device will have at least 3 degrees of threedom in in and out direction
	//additionally we neglect devices with a
	...
	//create the thread which runs a driver update loop which collects the data from the CHAI3D device
...
bool VistaCHAI3DHapticDevicesDriver::DoSensorUpdate(VistaType::microtime nTs)
{
	...
	VistaCHAI3DHapticDevicesMeasures::sCHAI3DHapticDevicesMeasure *s = reinterpret_cast<VistaCHAI3DHapticDevicesMeasures::sCHAI3DHapticDevicesMeasure*>(&( *pM ).m_vecMeasures[0]);
	...
	//fill into a vista transformation matrix under consideration of different coordinate szstem of chai3D
	...
	//up to now we only got the rotation, but m_afRotMatrix should contain the full transformation matrix
...
bool VistaCHAI3DHapticDevicesDriver::PhysicalEnable(bool bEnable)
{
	if(bEnable)
	{
		if( m_pUpdateLoop->IsPausing() ) m_pUpdateLoop->UnpauseThread();
		else if( !(m_pUpdateLoop->IsRunning()) ) m_pUpdateLoop->Run();
		return true;
	}
	else
	{
		if( m_pUpdateLoop->IsRunning() )  m_pUpdateLoop->StopGently(true);
		return true;
	}
}
...
VistaCHAI3DHapticDevicesDriver::VistaCHAI3DHapticDevicesForceFeedbackAspect::VistaCHAI3DHapticDevicesForceFeedbackAspect( VistaCHAI3DHapticDevicesDriver *pDriver )
...
VistaCHAI3DHapticDevicesDriver::VistaCHAI3DHapticDevicesForceFeedbackAspect::~VistaCHAI3DHapticDevicesForceFeedbackAspect()
{
}

bool VistaCHAI3DHapticDevicesDriver::VistaCHAI3DHapticDevicesForceFeedbackAspect::SetForce( const VistaVector3D   & v3Force,
...
int VistaCHAI3DHapticDevicesDriver::VistaCHAI3DHapticDevicesForceFeedbackAspect::GetNumOutputDOF() const
{
	return m_nOutputDOF;
}

float VistaCHAI3DHapticDevicesDriver::VistaCHAI3DHapticDevicesForceFeedbackAspect::GetMaximumStiffness() const
{
    return m_nMaxStiffness;
}

float VistaCHAI3DHapticDevicesDriver::VistaCHAI3DHapticDevicesForceFeedbackAspect::GetMaximumForce() const
{
    return m_nMaxForce;
}