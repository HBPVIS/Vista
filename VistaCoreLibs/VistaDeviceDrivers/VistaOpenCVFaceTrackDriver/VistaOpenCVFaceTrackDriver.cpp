...
/*                                                                            */
/*============================================================================*/
// $Id: VistaOpenCVFaceTrackDriver.cpp 5761 2011-01-17 19:46:22Z ingoassenmacher $
...
VistaOpenCVFaceTrackDriverCreationMethod::VistaOpenCVFaceTrackDriverCreationMethod( IVistaTranscoderFactoryFactory *pFac )
: IVistaDriverCreationMethod( pFac)
{
	RegisterSensorType( "FACE",
		sizeof( VistaOpenCVFaceTrackDriver::FacePoseMeasure ),
...
typedef TParameterCreate<VistaOpenCVFaceTrackDriver, VistaOpenCVFaceTrackDriver::FaceTrackParameters> paramCreator;
...
VistaOpenCVFaceTrackDriver::FaceTrackParameters::FaceTrackParameters( VistaOpenCVFaceTrackDriver* pDriver )
...
bool VistaOpenCVFaceTrackDriver::FaceTrackParameters::SetClassifierFile( const std::string& sFilename )
{
...
, m_pImage( NULL )
{
	...
	pSensor->SetMeasureTranscode( pCreationMethod->GetTranscoderFactoryForSensor( "FACE" )->CreateTranscoder() );
	pSensor->SetTypeHint( GetFactory()->GetTypeFor( "FACE" ) );
...
VistaOpenCVFaceTrackDriver::~VistaOpenCVFaceTrackDriver()
{
	...
	GetFactory()->GetTranscoderFactoryForSensor("FACE")->DestroyTranscoder( pSensor->GetMeasureTranscode() );
...
/* IMPLEMENTATION                                                             */
/*============================================================================*/
bool VistaOpenCVFaceTrackDriver::Connect()
{
...
void VistaOpenCVFaceTrackDriver::ConnectToOpenCVDevice()
{
	...
	if( iID >= 0 && iID < CLEyeGetCameraCount() )
	{
		...
		{
			if( ( nResolution == CLEYE_QVGA
					&& ( nReqFPS == 125 || nReqFPS == 100 ) )
				|| ( nResolution == CLEYE_QVGA && ( nReqFPS == 40 ) )
				|| nReqFPS == 75 || nReqFPS == 60 || nReqFPS == 30 || nReqFPS == 25 )
			{
				nActualFPS = nReqFPS;
			}
			else
...
void VistaOpenCVFaceTrackDriver::UpdateEyeDistanceFactor()
{
	...
	m_nEyeDistanceFactor = pParams->GetNormalizedEyeDistance()
						* pParams->GetEyeDistance();
