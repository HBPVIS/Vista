...
REFL_IMPLEMENT_FULL(VistaOpenALRecordDriver::Parameters,
			   VistaDriverGenericParameterAspect::IParameterContainer);


VistaOpenALRecordDriverCreationMethod::VistaOpenALRecordDriverCreationMethod(IVistaTranscoderFactoryFactory *fac)
: IVistaDriverCreationMethod(fac)
{
	...
	RegisterSensorType( "AUDIO",
		8820, // default block size for one slot is 2205(+1) bytes (assume 16bit mono / 44100 kHz -> 100ms)
...
template<class T>
bool propChange( VistaOpenALRecordDriver *driver,
				 IVistaReflectionable *props,
		         T &value, const T &toSet, int msg, bool def=false )
{
	if( value != toSet )
	{
		value = toSet;
...
//	const std::string SsReflectionName("VistaOpenALRecordDriver::Parameters");
	...
	IVistaPropertyGetFunctor *SaParameterGetter[] =
	{
		...
		"FREQUENCY",
		SsReflectionName,
		&VistaOpenALRecordDriver::Parameters::GetRecordingFrequency,
		...
		"BUFFERTIME",
		SsReflectionName,
		&VistaOpenALRecordDriver::Parameters::GetBufferTime,
		...
		"SAMPLES",
		SsReflectionName,
		&VistaOpenALRecordDriver::Parameters::GetNumberOfSamples,
	...
	public:
		...
		virtual bool PostPoll() { return true; }
	};
...
{
	return propChange<std::string>( m_parent, this, m_strDeviceName, strName, MSG_DEVICENAME_CHG, m_isDef );
}
...
, numBytesPerSample(-1)
{
	...
	pSensor->SetTypeHint( GetFactory()->GetTypeFor( "AUDIO" ) );
	...
	m_pThread->SetDriverUpdatePrepare( new TimeoutBasedOnProps( m_pParams->GetParameter<VistaOpenALRecordDriver::Parameters>() ) );
	...
	m_pInfo->GetInfoPropsWrite().SetProperty(
			 VistaProperty( "DEFAULTDEVICE",
					        OpenALSoundContext::OpenALCaptureContext::getDefaultCaptureDevice()));
}
...
VistaOpenALRecordDriver::~VistaOpenALRecordDriver()
{
	...
	GetFactory()->GetTranscoderFactoryForSensor("AUDIO")->DestroyTranscoder( pSensor->GetMeasureTranscode() );
	RemDeviceSensor(pSensor);
	delete pSensor;
...
/* IMPLEMENTATION                                                             */
/*============================================================================*/
bool VistaOpenALRecordDriver::Connect()
{
	if( m_pContext )
