...
/*============================================================================*/
/*
 * $Id: OpenALSoundContext.cpp,v 1.10 2008-10-30 17:59:33 av006ai Exp $
 */
...
OpenALSoundContext::~OpenALSoundContext()
{
	for(std::map<std::string, OpenALSoundContext::OpenALSoundEnvironment*>::iterator it = m_mpEnvs.begin();
		it != m_mpEnvs.end(); ++it)
	{
		delete (*it).second;
	}
...
OpenALSoundContext::OpenALSoundEnvironment *OpenALSoundContext::CreateAndAddSoundEnv(const std::string &sEnvName)
{
...
OpenALSoundContext::OpenALSoundEnvironment *OpenALSoundContext::GetSoundEnvironment(const std::string &sEnvName) const
{
...
{
	delete m_pListener;
	for(std::vector<OpenALVirtualSound*>::iterator it = m_mpSounds.begin();
		it != m_mpSounds.end(); ++it)
	{
		delete *it;
	}
...
OpenALSoundContext::OpenALVirtualListener *OpenALSoundContext::OpenALSoundEnvironment::GetListener() const
{
	return m_pListener;
}
...
OpenALSoundContext::OpenALVirtualSound *OpenALSoundContext::OpenALSoundEnvironment::CreateAndAddSound()
{
...
{
	std::vector<OpenALVirtualSound*>::iterator it = std::find(m_mpSounds.begin(),
		                                                          m_mpSounds.end(),
																  vs);
...
bool OpenALSoundContext::OpenALVirtualListener::UpdateListenerWCS(float pos[3],
	                   float view[3],
					   float up[3])
{
	return UpdateListenerWCS(pos[0], pos[1], pos[2],
...
bool OpenALSoundContext::OpenALVirtualListener::UpdateListenerWCSView(float viewx, float viewy, float viewz)
{
	float up[6];
	m_pEnv->MakeCurrent();
...
bool OpenALSoundContext::OpenALVirtualListener::UpdateListenerWCSUp(float upx, float upy, float upz)
{
	float up[6];
	m_pEnv->MakeCurrent();
	alGetListenerfv(AL_ORIENTATION, up);
...
bool OpenALSoundContext::OpenALVirtualListener::UpdateListenerVel(float velx, float vely, float velz)
{
	m_pEnv->MakeCurrent();
...
OpenALSoundContext::OpenALSoundEnvironment *OpenALSoundContext::OpenALVirtualListener::GetEnvironment() const
{
	return m_pEnv;
}
...
: m_pEnv(pEnv), m_nMuteGain(-1), m_ePlayState(PS_NONE)
{
	m_pEnv->MakeCurrent();
...
OpenALSoundContext::OpenALSound &OpenALSoundContext::OpenALSound::operator=(const OpenALSoundContext::OpenALSound &other)
{
	m_pEnv = other.m_pEnv;
	m_nSndId = other.m_nSndId;
	m_nMuteGain = other.m_nMuteGain;
...
void  OpenALSoundContext::OpenALSound::GetFloat3Array(int nId, float &f1, float &f2, float &f3) const
{
	m_pEnv->MakeCurrent();
...
OpenALSoundContext::OpenALVirtualSound::eState OpenALSoundContext::OpenALSound::GetSoundState() const
{
...
OpenALSoundContext::OpenALSoundEnvironment *OpenALSoundContext::OpenALSound::GetEnvironment() const
{
	return m_pEnv;
}
...
OpenALSoundContext::OpenALVirtualSound &OpenALSoundContext::OpenALVirtualSound::operator=(const OpenALSoundContext::OpenALVirtualSound &other)
{
...
OpenALSoundContext::OpenALBuffer &OpenALSoundContext::OpenALBuffer::operator=(const OpenALBuffer &other)
{
	if(this == &other)
		return *this;
...
OpenALSoundContext::OpenALBuffer *OpenALSoundContext::OpenALBuffer::ReadFromWav(const std::string &sFileName)
{
...
bool OpenALSoundContext::OpenALBuffer::FillBuffer(ALuint nSize,
							            ALuint freq,
...
OpenALSoundContext::OpenALCaptureContext::OpenALCaptureContext( size_t freq,
		             unsigned int format,
