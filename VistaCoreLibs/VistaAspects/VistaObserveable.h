...
/**
 * This is an interface class for a basic implementation of a simple oberver-pattern
 * as presented by Gamma et.al.
 * This class does define the interface for observeables, meaning entities that can change
 ...
 * outlive its observeables in this implementation, so there has to be a deletion scheme that
 * assures that there are no dangling pointers in different observers.
 * For that reason, this class does implement a deletion scheme that does inform all attached
 * observers about the deletion. Note that the clearing of stored pointers in part of the
 * observer implementation.
 * One thing to notice about event notifications and change sets:
 * - you can turn off notification when doing a series of updates (called 'set-run')
 ...
 * - a call to the destructor of an observeable will call IVistaObserver::ObserveableDelete()
	 ...
	 This does especially mean not to downcast the observeable pointer to anything else than
	 an IVistaObserveable itself!
 * - it surely is a wise thing to provide a de-registration for observeables besided the call
...
public:
	enum
	{
		...
		MSG_ATTACH,    /**< will be sent on a new attach  of an observer (using AttachObserver())*/
		MSG_DETACH,    /**< will be sent after an observer detached (using DetachObserver())*/
		MSG_PREDELETE, /**< can be send before deletion (by user code!)*/
	...
	/**
	 * Removes all occurences of pObs in the associated list of observers.
	 ...
	 * the parameter eMsg is collected in the change set for this observeable.
	...
	/**
	 * This cethod can be used to temporarily switch notification on or off
	...
	/**
	 * Copy constructor, it does omit the list of attached observers.
	 * As the observes often do bookkeeping of their attached observeables,
	 * this is the safest way of copying a subject.
	 * The observeable that is created by using this copy operation is thus
	...
	/**
	 * Assignment operator, it does omit the list of attached observers.
	 * As the observes often do bookkeeping of their attached observeables,
	 * this is the safest way of copying a subject.
	 * The observeable that is created by using this assignment is thus
	 * not observed by any observer, has an empty change set,
	 * but notification state is the same as in the original.
	 */
...
protected:
	...
	/**
	 * An observeable can be deleted, in order to assure that no observer will keep references
