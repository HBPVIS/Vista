...
namespace
{
	...
	IVdfnNode *CreateNode( TiXmlElement *node,
						   const VdfnNodeFactory &fac,
						   ...
						   )
	{
			...
			if( nodeParams.HasProperty( pcName ) )
			{
				vstr::outi() << "[VdfnNode::CreateNode]: External props found for node ["
				          ...
				          << "]"
				          << std::endl;
				if( nodeParams.GetPropertyConstRef(pcName).GetPropertyType() == VistaProperty::PROPT_PROPERTYLIST ) // does it have a subsection for this node? and is it a PropertyList?
	...
	VdfnGraph *CreateGraph( TiXmlHandle &nodespace,
							 TiXmlHandle &graph,
							 TiXmlHandle &edges,
							 const VdfnNodeFactory &fac,
							 const std::string &strTag,
						   bool bEvaluate,
						   bool &bContainsErrors,
...
//				std::cerr << "]" << std::endl;
    ...
    std::string CleanNodeName( const std::string &strNodeName )
    {
        ...
        for(std::string::iterator cit = strOut.begin();
            cit != strOut.end(); ++cit)
        {
            if( (*cit) == '-' || (*cit) == ' ')
                (*cit) = '_';
    ...
    {
    	std::string strOut;
        for(std::string::const_iterator cit = strIn.begin();
             cit != strIn.end(); ++cit)
         {
        	if( (*cit) == '<' || (*cit) == '>' )
        		strOut.push_back('\\');
    ...
    std::string NoSpacing( const std::string &strNodeName )
    {
        ...
        for(std::string::const_iterator cit = strNodeName.begin();
            cit != strNodeName.end(); ++cit)
        {
            if( (*cit) == ' ')
            {
                strOut.push_back( '\\' );
     ...
     */
	bool DumpXml(TiXmlHandle &nodespace,
			     TiXmlHandle &graph,
			     TiXmlHandle &edges,
			     const std::string &strDotFile,
			     const std::string &strGraph )
	{
		// open dot file
        ...
        // ok, lets dance...
		of  << "digraph " << strGraphName << std::endl
			<< "{" << std::endl;
	 ...
	 * contain graphs.
	 */
	bool CreateExports( const TiXmlHandle &oExportRoot,
			            ...
			            bool bStrict )
	{
		...
		for( ;graphel != NULL; graphel = graphel->NextSiblingElement("export") )
		{
			...
			{
				liExports.push_back( VdfnGraph::ExportData( sPrefix, sPostfix, sMapName, VdfnGraph::ExportData::INPORT ));
			}
			else
			{
				liExports.push_back( VdfnGraph::ExportData( sPrefix, sPostfix, sMapName, VdfnGraph::ExportData::OUTPORT ));
			}
		}
		                       ...
		                       bool bStrict)
{
	...
	if(loadOkay)
	{
		...
		if( module.Element() && nodes.Element() && graph.Element() && edges.Element() )
		{
			...
			VdfnGraph *pGraph = CreateGraph( nodes,
								graph,
								edges,
								*VdfnNodeFactory::GetSingleton(),
								strGraphTag,
							  bEvaluateGraph,
							  bContainsErrors);
...
#else
					int state;
					char *ret = abi::__cxa_demangle(pNode->GetOutPort(*it)->GetTypeDescriptor().c_str(), NULL, NULL, &state);
					if( state == 0 && ret )
					{
						str	<< NoSpacing(EscapeBraces(ret));
						free(ret);
...
//                str << strPortName.c_str() << " [style=filled,fillcolor=green,shape=box,label=\""
//                    << *it << "\\n("
...
//
//                // and directly write an edge between the node and the port
//                str << strPortName.c_str() << " [style=filled,color=yellow,fillcolor=yellow,shape=box,label=\""
//                    << *tit << "\\n("
								  ...
								  bool bWritePorts, bool bWriteTypes, bool bWriteValues)
{
	...
	if(str.good())
	{
        ...
        // ok, lets dance...
		str << "digraph " << strGraphName << std::endl
			<< "{" << std::endl;
...
//																	+ strFromPortName);
							...
							str << "\t" << strNodeName << ":O_" << CleanNodeName(strFromPortName)<< " -> "
								<< CleanNodeName((*nit).first->GetNameForNameable()) << ":I_" << CleanNodeName(strToPortName)
								<< " [label=\"" << strToPortName << "\"];\n";
						}
						else
						{
