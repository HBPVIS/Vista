...
public:
	VdfnNoMatchCompare() {}
	virtual bool IsTypeOf( const IVdfnPort * ) const { return false; }
	virtual IVdfnPortTypeCompare *Clone() const { return new VdfnNoMatchCompare; }
	virtual bool Assign( IVdfnPort * ) { return false; }
...
IVdfnNode::~IVdfnNode()
{
	for(PortMap::iterator it1 = m_mpOutPorts.begin();
		it1 != m_mpOutPorts.end(); ++it1)
	{
		delete (*it1).second;
	}

	for(TypeMap::iterator it2 = m_mpInPrototypes.begin();
		it2 != m_mpInPrototypes.end(); ++it2)
	{
		delete (*it2).second;
	}
...
bool IVdfnNode::GetNameForInPort( IVdfnPort *pPort, std::string &strName ) const
{
	// reverse lookup in m_mpInPortMap
	for(PortMap::const_iterator cit = m_mpInPorts.begin();
...
{
	// reverse lookup in m_mpInPortMap
	for(PortMap::const_iterator cit = m_mpOutPorts.begin();
		cit != m_mpOutPorts.end(); ++cit)
	{
		if( (*cit).second == pPort )
		{
			strName = (*cit).first;
...
{
	std::list<std::string> liRet;
	for(TypeMap::const_iterator cit = m_mpInPrototypes.begin();
		cit != m_mpInPrototypes.end(); ++cit)
	{
		liRet.push_back( (*cit).first );
	}
...
IVdfnPortTypeCompare *IVdfnNode::CheckInPortRegistration( const std::string &sName, IVdfnPort *pPort ) const
{
