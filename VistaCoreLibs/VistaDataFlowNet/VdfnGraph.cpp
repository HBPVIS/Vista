...
VdfnGraph::~VdfnGraph()
{
	for( Nodes::const_iterator cit = m_liNodes.begin();
		cit != m_liNodes.end(); ++cit )
	{
		delete (*cit);
	}
}
...
{
	for(NodeVec::const_iterator it = m_vecTraversal.begin(); it != m_vecTraversal.end(); ++it)
	{
		// iterate over source nodes ONLY
		// these are nodes with no attached input (even if they have one, in therory)
...
bool VdfnGraph::EvaluateGraph(double nTimeStamp)
{
	...
	for( ritReEvalNode = m_mpReEvalSubgraphs.rbegin();
		ritReEvalNode != m_mpReEvalSubgraphs.rend(); /* no loop increment condition */)
	{
		...
		// The tests and reevaluations are performed back-to-front in order
...
{
	for( NodeVec::const_iterator citNode = vecSubGraph.begin();
		citNode != vecSubGraph.end(); ++citNode)
	{
...
bool VdfnGraph::GetIsConnectedFromTo( IVdfnNode *pFrom, IVdfnNode *pTo) const
{
	...
	for(std::list<Connect>::const_iterator t = liPrts.begin();
		t != liPrts.end(); ++t )
	{
		const ConInfo &edges = (*t).second;
		for(ConInfo::const_iterator nit = edges.begin();
			nit != edges.end(); ++nit)
		{
			if( (*nit).first == pTo )
				return true;
...
bool VdfnGraph::ReloadActionObjects()
{
	for(Nodes::iterator uit = m_liNodes.begin(); uit != m_liNodes.end(); ++uit )
	{
		...
		for(std::list<std::string>::const_iterator sit = liOutPorts.begin();
			sit != liOutPorts.end(); ++sit)
		{
		...
		cit != m_liNodes.end(); ++cit)
	{
		...
		for(std::list<std::string>::const_iterator sit = liInPorts.begin();
			sit != liInPorts.end(); ++sit)
		{
			...
			if(pPort) // maybe NULL iff not connected
			{
				...
				if( pit != m_mpPortLookup.end() )
				{
					...
					Connect &edges = GetAdjacencyList( (*pit).second, pit->first );
					edges.second.push_back( std::pair<IVdfnNode*, IVdfnPort*>(*cit, pPort) ); // add edge from (in_p) -> (sink_p)
				}
			}
...
bool VdfnGraph::UpdateTraversalVector()
{
	...
	for(Edges::iterator cit = m_mpEdges.begin(); cit != m_mpEdges.end(); ++cit)
	{
			...
			t != liPrts.end(); ++t )
		{
			...
			//		ConInfo &liChildren = (*cit).second.second;

			// add node with children to topology graph
			for( ConInfo::const_iterator nit = liChildren.begin();
				nit != liChildren.end(); ++nit)
			{
		...
		tcit != liNodes.end(); ++tcit)
	{
		...
		//check if node is a reevalnode. if so, create its subgraph
		...
		if( pReEvalNode )
		{
			...
			std::list<VistaTopologyGraph<IVdfnNode*>::Node*>::const_iterator citOrigNode = liSubgraph.begin();
			std::vector<IVdfnNode*>::iterator itCopyNode = vecSubgraph.begin();
			for( ; citOrigNode != liSubgraph.end(); ++citOrigNode, ++itCopyNode )
	...
	for(Nodes::const_iterator ucit = m_liNodes.begin(); ucit != m_liNodes.end(); ++ucit )
	{
		if( !topology.GetNodeByValue( *ucit ) ) // not in topology graph, so it is not in the edge list
		{
			// while we are at it: call initialize on these nodes :)
			if( (*ucit)->PrepareEvaluationRun() == true )
				m_vecTraversal.push_back( *ucit ); // order is not important, could be anywhere in the list
			else
			{
				// omit broken nodes
...
bool VdfnGraph::UpdatePortLookupMap()
{
	// clear old values
...
VdfnGraph::Connect &VdfnGraph::GetAdjacencyList( IVdfnNode *pNode, IVdfnPort *pPort )
{
	...
	else
	{
		...
		for(std::list<Connect>::iterator cit = (*it).second.begin();
...
VdfnGraph::Nodes VdfnGraph::GetSourceNodes() const
{
	...
	if(!m_vecTraversal.empty())
	{
		for(NodeVec::const_iterator cit = m_vecTraversal.begin();
			cit != m_vecTraversal.end(); ++cit)
		{
...
void VdfnGraph::SetIsActive( bool bIsActive, double dTimeStamp )
{
	...
	for(Nodes::iterator it = m_liNodes.begin();
		it != m_liNodes.end(); ++it)
	{
		if(!(*it)->GetIsValid())
			continue;
