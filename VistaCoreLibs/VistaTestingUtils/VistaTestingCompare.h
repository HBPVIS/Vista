...
namespace VistaTesting
{
	...
	struct CompareObject
	{
		::testing::AssertionResult operator() ( const T& oLeft, const T& oRight,
												const double nMaxRelDeviation = 0, 
												...
												const double nMinRelevantSize = 0  )
	{
		if( oLeft == oRight )
			return ::testing::AssertionSuccess() << oLeft << " == " << oRight;
		if( std::abs( oLeft ) < (TFloat)nMinRelevantSize 
			&& std::abs( oLeft ) < (TFloat)nMinRelevantSize )
		{
			return ::testing::AssertionSuccess() << oLeft << " && " << oRight
								<< " below min of " << (TFloat)nMinRelevantSize;
		}
	...
	{
		::testing::AssertionResult operator() ( const VistaColor& oLeft,
												const VistaColor& oRight,
												const double nMaxRelDeviation = 0, 
												const double nMinRelevantSize = 0  ) const
		{
			for( int nAxis = Vista::X; nAxis <= Vista::W; ++nAxis )
			{
				::testing::AssertionResult oRes = Compare( oLeft[nAxis], oRight[nAxis],
												nMaxRelDeviation, nMinRelevantSize );
				if( oRes == false )
	...
	{
		::testing::AssertionResult operator() ( const VistaVector3D& oLeft,
	...
	{
		::testing::AssertionResult operator() ( const VistaQuaternion& oLeft,
												...
												const double nMinRelevantSize = 0  ) const
		{
			for( int nAxis = Vista::X; nAxis <= Vista::W; ++nAxis )
			{
				...
				if( oRes == false )
				{
					...
					::testing::AssertionResult oRes1 = Compare( std::abs( oLeft[nAxis] ), 1.0f,
													nMaxRelDeviation, nMinRelevantSize );
					::testing::AssertionResult oRes2 = Compare( std::abs( oRight[nAxis] ), 1.0f,
													nMaxRelDeviation, nMinRelevantSize );
					if( oRes1 && oRes2 )
					{
	...
	struct CompareObject< VistaTransformMatrix >
	{
												...
												const double nMinRelevantSize = 0  ) const
		{
			for( int nRow = Vista::X; nRow <= Vista::W; ++nRow )
			{
				...
				{
					::testing::AssertionResult oRes = Compare( oLeft( nRow, nCol ), oRight( nRow, nCol ),
															nMaxRelDeviation, nMinRelevantSize );
					if( oRes == false )
						return oRes << " ( VistaTransformMatrix entry ( " << nRow
									<< ", " << nCol << " ) )";
				}				
			}
			return ::testing::AssertionSuccess();
	...
	struct CompareObject< std::pair< T1, T2 > >
	{
		::testing::AssertionResult operator() ( const std::pair< T1, T2 >& oLeft,
												const std::pair< T1, T2 >& oRight,
												const double nMaxRelDeviation = 0, 
												const double nMinRelevantSize = 0  ) const
		{
			::testing::AssertionResult oResFirst = Compare( oLeft.first, oRight.first,
															nMaxRelDeviation, nMinRelevantSize );
			if( oResFirst == false )
			{
				return oResFirst << " " << VistaConversion::GetTypeName< std::pair< T1, T2 > >()
								<< ".first";
			}
			::testing::AssertionResult oResSecond = Compare( oLeft.second, oRight.second,
															nMaxRelDeviation, nMinRelevantSize );
			if( oResSecond == false )
			{
				return oResSecond << " " << VistaConversion::GetTypeName< std::pair< T1, T2 > >()
								<< ".second";
			}
			return ::testing::AssertionSuccess();
		}
	...
	template< typename TContainer >
	::testing::AssertionResult CompareTopPopContainer( const TContainer& oLeft,
												const TContainer& oRight,
												const double nMaxRelDeviation = 0, 
												const double nMinRelevantSize = 0 )
	{
		...
		{
			::testing::AssertionResult oResult = Compare( oLeftCopy.top(),
														oRightCopy.top(),
														nMaxRelDeviation, nMinRelevantSize );
			if( oResult == false )
			{
												...
												const double nMinRelevantSize = 0 )
	{
		...
		while( oLeftCopy.empty() == false )
		{
			::testing::AssertionResult oResult = Compare( oLeftCopy.front(),
														oRightCopy.front(),
														nMaxRelDeviation, nMinRelevantSize );
			if( oResult == false )
			{
	...
	struct CompareObject< VistaProperty >
	{
												...
												const double nMinRelevantSize = 0  ) const
		{
			...
			switch( oLeft.GetPropertyType() )
			{
				...
				{
					::testing::AssertionResult oResValue = Compare( oLeft.GetValueConstRef(),
																oRight.GetValueConstRef(),
																nMaxRelDeviation, nMinRelevantSize );
					if( oResValue == false )
				...
				case VistaProperty::PROPT_INT:
				{
					::testing::AssertionResult oResValue = Compare(
											VistaConversion::FromString<int>( oLeft.GetValueConstRef() ),
											VistaConversion::FromString<int>( oRight.GetValueConstRef() ),
											nMaxRelDeviation, nMinRelevantSize );
					if( oResValue == false )
						return oResValue << " Property value";
					break;
				...
				{
					::testing::AssertionResult oResValue = Compare( 
											VistaConversion::FromString<bool>( oLeft.GetValueConstRef() ),
											VistaConversion::FromString<bool>( oRight.GetValueConstRef() ),
											nMaxRelDeviation, nMinRelevantSize );
					if( oResValue == false )
						return oResValue << " Property value";
					break;
				...
				{
					::testing::AssertionResult oResValue = Compare( 
											VistaConversion::FromString<double>( oLeft.GetValueConstRef() ),
											VistaConversion::FromString<double>( oRight.GetValueConstRef() ),
											nMaxRelDeviation, nMinRelevantSize );
					if( oResValue == false )
						return oResValue << " Property value";
					break;
				...
				case VistaProperty::PROPT_LIST:
				{
					...
					switch( oLeft.GetPropertyListSubType() )
					{
						...
						case VistaProperty::PROPT_INT:
						{
							...
							::testing::AssertionResult oResList = Compare( vecLeft, vecRight,
													nMaxRelDeviation, nMinRelevantSize );
							if( oResList == false )
				...
				{
					::testing::AssertionResult oResName = Compare( oLeft.GetPropertyListConstRef(),
																oRight.GetPropertyListConstRef(),
																nMaxRelDeviation, nMinRelevantSize );
					if( oResName == false )
						return oResName << " Property value sub proplist";
					break;
				}
				default:
					VISTA_THROW( "Invalid Property list type", -1 );
