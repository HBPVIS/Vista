...
#if !(defined(IRIX) || defined(DARWIN))
static int wcFilter(const struct dirent *dir_entry)
#else
// we need to remove the const qualifier for IRIX
static int wcFilter(struct dirent *dir_entry)
#endif
{
	if(scPattern)
...
int VistaFileSystemDirectory::ReadEntries()
{
	if (!m_bIsParsed) {
		...
		scPattern = (char*)m_sPattern.c_str();
