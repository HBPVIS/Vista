...
void VistaRandomNumberGenerator::SetSeedByArray(unsigned int init_key[], int key_length)
{
	...
	for (; k; k--) {
		m_mt[i] = (m_mt[i] ^ ((m_mt[i-1] ^ (m_mt[i-1] >> 30)) * 1664525))
	...
	for (k=N-1; k; k--) {
		m_mt[i] = (m_mt[i] ^ ((m_mt[i-1] ^ (m_mt[i-1] >> 30)) * 1566083941))
		- i; /* non linear */
		m_mt[i] &= 0xffffffff; /* for WORDSIZE > 32 machines */
		i++;
		if (i>=N) { m_mt[0] = m_mt[N-1]; i=1; }
...
unsigned int VistaRandomNumberGenerator::GenerateInt32(unsigned int iLowerBound, unsigned int iUpperBound)
{
