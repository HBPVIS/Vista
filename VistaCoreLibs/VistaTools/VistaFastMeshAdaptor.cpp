...
/*                                                                            */
/*============================================================================*/
// $Id: VistaFastMeshAdaptor.cpp 21315 2011-05-16 13:47:39Z dr165799 $
...
VistaFastMeshAdaptor::VistaFastMeshAdaptor(const unsigned int iNumberOfVertices,
											 const std::vector<unsigned int> &vFaces)
: m_vIncidentFaces(iNumberOfVertices), m_vOppositeHalfedgeLinks(vFaces.size()), m_vFaces(vFaces)
{
	unsigned int i;
	...
	for (i=0; i<m_vFaces.size(); i+=3)
	{
		m_vOppositeHalfedgeLinks[i] = GetHalfedgeHandle(VertexHandle(m_vFaces[i+1]), VertexHandle(m_vFaces[i]));
		m_vOppositeHalfedgeLinks[i+1] = GetHalfedgeHandle(VertexHandle(m_vFaces[i+2]), VertexHandle(m_vFaces[i+1]));
		m_vOppositeHalfedgeLinks[i+2] = GetHalfedgeHandle(VertexHandle(m_vFaces[i]), VertexHandle(m_vFaces[i+2]));
	}
}
...
unsigned int
VistaFastMeshAdaptor::NumberOfVertices() const
{
	return (unsigned int)m_vIncidentFaces.size();
}

unsigned int
VistaFastMeshAdaptor::NumberOfFaces() const
{
...
bool
VistaFastMeshAdaptor::IsBoundary(const HalfedgeHandle hHalfedge) const
{
	assert((hHalfedge.Index() < m_vFaces.size())
		|| (hHalfedge.IsValid() == false));
...
bool
VistaFastMeshAdaptor::IsBoundary(const EdgeHandle hEdge) const
{
...
VistaFastMeshAdaptor::VertexHandle
VistaFastMeshAdaptor::GetToVertexHandle(const HalfedgeHandle hHalfedge) const
{
	assert(hHalfedge.Index() < m_vFaces.size());
...
VistaFastMeshAdaptor::VertexHandle
VistaFastMeshAdaptor::GetFromVertexHandle(const HalfedgeHandle hHalfedge) const
{
	assert(hHalfedge.Index() < m_vFaces.size());
	return VertexHandle(m_vFaces[hHalfedge.Index()]);
}

VistaFastMeshAdaptor::HalfedgeHandle
VistaFastMeshAdaptor::GetNextHalfedgeHandle(const HalfedgeHandle hHalfedge) const
{
	assert(hHalfedge.Index() < m_vFaces.size());
	if (hHalfedge.Index() % 3 != 2)
	{
...
VistaFastMeshAdaptor::HalfedgeHandle
VistaFastMeshAdaptor::GetPrevHalfedgeHandle(const HalfedgeHandle hHalfedge) const
{
	assert(hHalfedge.Index() < m_vFaces.size());
...
VistaFastMeshAdaptor::HalfedgeHandle
VistaFastMeshAdaptor::GetOppositeHalfedgeHandle(const HalfedgeHandle hHalfedge) const
{
	assert(hHalfedge.Index() < m_vFaces.size());
...
VistaFastMeshAdaptor::HalfedgeHandle
VistaFastMeshAdaptor::GetHalfedgeHandle(const VertexHandle hVertex) const
{
...
VistaFastMeshAdaptor::HalfedgeHandle
VistaFastMeshAdaptor::GetHalfedgeHandle(const VertexHandle hFromVertex, const VertexHandle hToVertex) const
{
	...
	for (it = m_vIncidentFaces[hFromVertex.Index()].begin(), it_end = m_vIncidentFaces[hFromVertex.Index()].end();
		 it != it_end;
		 ++it)
	{
...
VistaFastMeshAdaptor::HalfedgeHandle
VistaFastMeshAdaptor::GetHalfedgeHandle(const EdgeHandle hEdge) const
{
	assert(hEdge.Index() < m_vFaces.size());
...
VistaFastMeshAdaptor::HalfedgeHandle
VistaFastMeshAdaptor::GetHalfedgeHandle(const FaceHandle hFace) const
{
...
VistaFastMeshAdaptor::EdgeHandle
VistaFastMeshAdaptor::GetEdgeHandle(const HalfedgeHandle hHalfedge) const
{
	assert(hHalfedge.Index() < m_vFaces.size());
...
VistaFastMeshAdaptor::FaceHandle
VistaFastMeshAdaptor::GetFaceHandle(const HalfedgeHandle hHalfedge) const
{
...
VistaFastMeshAdaptor::ConstFaceIter
VistaFastMeshAdaptor::FacesBegin() const
{
	return ConstFaceIter(this, 0);
}

VistaFastMeshAdaptor::ConstFaceIter
VistaFastMeshAdaptor::FacesEnd() const
{
