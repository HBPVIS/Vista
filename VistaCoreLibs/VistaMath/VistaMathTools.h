...
/*                                                                            */
/*============================================================================*/
// $Id: VistaMathTools.h 20730 2011-03-30 15:56:24Z dr165799 $
...
{
	if (fabs(val) <= std::numeric_limits<Type>::epsilon())
		return true;
	return false;
}
...
bool VistaMathTools< std::complex<double> >::IsZero (const std::complex<double> &val)
{
	...
	if( fabs(val.real()) <= std::numeric_limits<double>::epsilon() )
#else
	if (std::fabs(val.real()) < (std::numeric_limits<double>::min)() /
	...
	if( fabs(val.real()) <= std::numeric_limits<double>::epsilon() )
#else
	if (std::fabs(val.imag()) < (std::numeric_limits<double>::min)() /
								std::numeric_limits<double>::epsilon())
#endif
	{
		compCount++;
...
bool VistaMathTools<Type>::IsZeroSetZero (Type & val)
{
	...
	if (fabs(val) <= std::numeric_limits<Type>::epsilon())
#else
	if (fabs(val) < (std::numeric_limits<Type>::min)() /
...
#if defined(_MSC_VER) && (_MSC_VER < 1400)
	if (fabs(val.imag()) <= std::numeric_limits<double>::epsilon())
#else
	if (std::fabs(val.imag()) < (std::numeric_limits<double>::min)() /
								std::numeric_limits<double>::epsilon())
#endif
