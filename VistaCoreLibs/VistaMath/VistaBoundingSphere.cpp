...
/*                                                                            */
/*============================================================================*/
// $Id: VistaBoundingSphere.cpp 21315 2011-05-16 13:47:39Z dr165799 $
...
const float VistaBoundingSphere::m_PI = 3.14159265358979323846f;
//============================================================================
//  MAKROS AND DEFINES
//============================================================================
	 ...
	 const float c[3])
{
	...
	const float fLengthAB = sqrt(fEdgeAB[0]*fEdgeAB[0] +
								 fEdgeAB[1]*fEdgeAB[1] +
								 fEdgeAB[2]*fEdgeAB[2]);
	const float fLengthBC = sqrt(fEdgeBC[0]*fEdgeBC[0] +
								 fEdgeBC[1]*fEdgeBC[1] +
								 fEdgeBC[2]*fEdgeBC[2]);
	const float fLengthCA = sqrt(fEdgeCA[0]*fEdgeCA[0] +
								 fEdgeCA[1]*fEdgeCA[1] +
								 fEdgeCA[2]*fEdgeCA[2]);

	// Compute the circumcenter.
	const float fCosinusA = (fEdgeAB[0]*fEdgeCA[0] +
							fEdgeAB[1]*fEdgeCA[1] +
							fEdgeAB[2]*fEdgeCA[2])/(-1.0f * fLengthAB * fLengthCA);
	const float fCosinusB = (fEdgeAB[0]*fEdgeBC[0] +
							fEdgeAB[1]*fEdgeBC[1] +
							fEdgeAB[2]*fEdgeBC[2])/(-1.0f * fLengthAB * fLengthBC);
	const float fCosinusC = (fEdgeBC[0]*fEdgeCA[0] +
							fEdgeBC[1]*fEdgeCA[1] +
							fEdgeBC[2]*fEdgeCA[2])/(-1.0f * fLengthBC * fLengthCA);
	...
	const float fRadiusVector[3] = {
		a[0] - m_center[0],
		a[1] - m_center[1],
		a[2] - m_center[2]
	};
	m_radius = (float)sqrt(fRadiusVector[0]*fRadiusVector[0] +
...
{
	return m_PI * 4.0f/3.0f * m_radius * m_radius * m_radius;
}
...
{ 
	const float fDistanceVector[3] = {
		...
		point[1] - m_center[1],
		point[2] - m_center[2]};
	return (fDistanceVector[0]*fDistanceVector[0]
...
bool VistaBoundingSphere::Intersects
	(const VistaBoundingSphere& BSphere) const
{
	const float fDistanceVector[3] = {
		m_center[0] - BSphere.m_center[0],
		m_center[1] - BSphere.m_center[1],
		m_center[2] - BSphere.m_center[2]
	};
	return (fDistanceVector[0]*fDistanceVector[0] +
			fDistanceVector[1]*fDistanceVector[1] +
...
VistaBoundingSphere VistaBoundingSphere::Include(const VistaBoundingSphere& BSphere)
{
	...
	if(fDistance2 > R*R)
	{
		...
		m_center[0] = (m_center[0] + BSphere.m_center[0] - (m_radius - BSphere.m_radius)*fDistanceVector[0]/fDistance)*0.5f;
		m_center[1] = (m_center[1] + BSphere.m_center[1] - (m_radius - BSphere.m_radius)*fDistanceVector[1]/fDistance)*0.5f;
		m_center[2] = (m_center[2] + BSphere.m_center[2] - (m_radius - BSphere.m_radius)*fDistanceVector[2]/fDistance)*0.5f;
	 ...
	 const float epsilon) const
{
	...
	const float fClosestPoint[3] = {
		origin[0] + fParametricValue * directionNorm[0],
		origin[1] + fParametricValue * directionNorm[1],
		origin[2] + fParametricValue * directionNorm[2]
	};
	const float fDistanceVector[3] = {
		m_center[0] - fClosestPoint[0],
		m_center[1] - fClosestPoint[1],
