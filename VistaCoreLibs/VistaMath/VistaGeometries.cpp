...
bool VistaRay::IntersectionTriangle(const VistaVector3D& a, 
...
bool VistaRay::IntersectionTriangle(const VistaVector3D& a, 
									 ...
									 const VistaVector3D& c,
									VistaVector3D& contactPoint, 
									VistaVector3D& faceNormal, 
									float& penetrationDepth, 
									float& barycentric_ab, 
									float& barycentric_ac,
									const float epsilon) const
{
	// The implementation is based on the algorithm from the book
	// Real-Time Rendering, M?ller & Haines, p. 303-305.
...
bool VistaLineSegment::IntersectionTriangle(const VistaVector3D& a, 
									 const VistaVector3D& b, 
									 const VistaVector3D& c,
									VistaVector3D& contactPoint, 
									const float epsilon) const
...
bool VistaLineSegment::ProximityContains(const VistaLineSegment &segment,
										  const float fProximity,
										  float& fBarycentricThisSegment,
										  float& fBarycentricOtherSegment,
										  const float fEpsilon) const
{
...
VistaSplineSegment::VistaSplineSegment (VistaVector3D & startPnt, VistaVector3D & endPnt,
										  ...
										  float & startParam, float & endParam,
										  int precision)
								   : VistaCurveSegment ( startPnt, endPnt ),
									 m_StartTangent(startTang), m_StartParameter(startParam),
									 m_EndTangent(endTang), m_EndParameter(endParam),
...
/*	int c,i,j,k;
	for(c=0; c<3; c++) // for each coordinate X, Y, Z
	{
		...
		float m[4][4] = {{(float)pow(endParam,3)     , (float)pow(endParam,2)   , endParam   , 1 },
					{(float)3*pow(endParam,2)   , (float)2*endParam        , 1          , 0},
					{(float)3*pow(startParam,2) , (float)2*startParam      , 1          , 0},
					{(float)pow(startParam,3)   , (float)pow(startParam,2) , startParam , 1}};
...
VistaVector3D  VistaSplineSegment::GetPointByParameter        ( const float          paramT ) const
{
	...
	float u = (float)((paramT-m_StartParameter)/(m_EndParameter-m_StartParameter));

	// calculate point's coordinates with hermite base matrix
	float h1=(2.0f*std::pow(u,3)-3.0f*std::pow(u,2)+1.0f);
	float h2=(-2.0f*std::pow(u,3)+3.0f*std::pow(u,2));
	float h3=(std::pow(u,3)-2.0f*std::pow(u,2)+u);
	float h4=(std::pow(u,3)-std::pow(u,2));

	for(int c=0;c<3;c++)
		point[c]=h1*m_StartPoint[c]+h2*m_EndPoint[c]+h3*m_StartTangent[c]+h4*m_EndTangent[c];

/*	point[Vista::X]=(float)(pow(param,3)*m_Coefs[Vista::X][0] + pow(param,2)*m_Coefs[Vista::X][1]+
			  m_Coefs[Vista::X][2]*param + m_Coefs[Vista::X][3]);
	point[Vista::Y]=(float)(pow(param,3)*m_Coefs[Vista::Y][0] + pow(param,2)*m_Coefs[Vista::Y][1]+
			  m_Coefs[Vista::Y][2]*param + m_Coefs[Vista::Y][3]);
	point[Vista::Z]=(float)(pow(param,3)*m_Coefs[Vista::Z][0] + pow(param,2)*m_Coefs[Vista::Z][1]+
			  m_Coefs[Vista::Z][2]*param + m_Coefs[Vista::Z][3]);*/
	return point;
}
...
// CONSTRUCTORS
VistaTriangle::VistaTriangle(const VistaVector3D& a, 
							   const VistaVector3D& b, 
							   const VistaVector3D& c)
: m_a(a), m_b(b), m_c(c)
{}
...
bool VistaTriangle::IntersectionPoint(const VistaVector3D& pnt, 
...
bool VistaTriangle::ProximityContains(const VistaVector3D& pnt,
									   const VistaVector3D& triangleNormal,
									   const float fProximityDistance,
									   float& ab, float& ac,
									   const float fEpsilon) const
{
	// Check if point is close enough (in normal direction) to the triangle
	if (std::fabs(triangleNormal * (pnt-m_a)) > fProximityDistance - fEpsilon)
	{
		return false;
	}
	...
	const float scale = ((m_a - m_b).GetLength() + (m_b-m_c).GetLength() + (m_c-m_a).GetLength())/3.0f;
	const float delta = fProximityDistance/scale + fEpsilon;
	if ((-delta < a) && (a <1.0f+delta)
		&& (-delta < b) && (b <1.0f+delta)
		&& (-delta < c) && (c <1.0f+delta))
	{
		ab = b;
		ac = c;
		return true;
...
bool VistaTriangle::Intersects(const VistaTriangle& tri)
{
	...
	a1[0] = m_a[0]; a1[1] = m_a[1];	a1[2] = m_a[2];
	b1[0] = m_b[0]; b1[1] = m_b[1];	b1[2] = m_b[2];
	c1[0] = m_c[0]; c1[1] = m_c[1];	c1[2] = m_c[2];
	a2[0] = tri.m_a[0]; a2[1] = tri.m_a[1];	a2[2] = tri.m_a[2];
	b2[0] = tri.m_b[0]; b2[1] = tri.m_b[1];	b2[2] = tri.m_b[2];
	c2[0] = tri.m_c[0]; c2[1] = tri.m_c[1];	c2[2] = tri.m_c[2];
...
bool VistaTriangle::CheckMinMax(const VistaVector<double,3>& p1, const VistaVector<double,3>& q1, const VistaVector<double,3>& r1,
...
bool VistaTriangle::IntersectionTriTri3D(const VistaVector<double,3>& p1, const VistaVector<double,3>& q1, const VistaVector<double,3>& r1,
								const VistaVector<double,3>& p2, const VistaVector<double,3>& q2, const VistaVector<double,3>& r2,
...
bool VistaTriangle::Coplanar(const VistaVector<double,3>& p1, const VistaVector<double,3>& q1, const VistaVector<double,3>& r1, 
							const VistaVector<double,3>& p2, const VistaVector<double,3>& q2, const VistaVector<double,3>& r2, 
							const VistaVector<double,3>& normal1, const VistaVector<double,3>& normal2)
{
	const double n_x = std::fabs(normal1[0]);
	const double n_y = std::fabs(normal1[1]);
	const double n_z = std::fabs(normal1[2]);
...
bool VistaTriangle::Intersection2D(const VistaVector<double,2>& p1, const VistaVector<double,2>& q1, const VistaVector<double,2>& r1,
...
bool VistaTriangle::Intersection2DCCW(const VistaVector<double,2>& p1, const VistaVector<double,2>& q1, const VistaVector<double,2>& r1,
								  const VistaVector<double,2>& p2, const VistaVector<double,2>& q2, const VistaVector<double,2>& r2)
{
...
double VistaTriangle::Orientation2D(const VistaVector<double,2>& a, const VistaVector<double,2>& b, const VistaVector<double,2>& c)
{
	return (a[0]-c[0])*(b[1]-c[1])-(a[1]-c[1])*(b[0]-c[0]);
}


bool VistaTriangle::IntersectionVertex(const VistaVector<double,2>& P1, const VistaVector<double,2>& Q1, const VistaVector<double,2>& R1,
											  const VistaVector<double,2>& P2, const VistaVector<double,2>& Q2, const VistaVector<double,2>& R2)
{
...
bool VistaTriangle::IntersectionEdge(const VistaVector<double,2>& P1, const VistaVector<double,2>& Q1, const VistaVector<double,2>& R1,
											const VistaVector<double,2>& P2, const VistaVector<double,2>& Q2, const VistaVector<double,2>& R2)
{
	if (Orientation2D(R2,P2,Q1) >= 0.0)
	{
...
VistaPlane::VistaPlane(
		...
		const VistaVector3D& xDir, const VistaVector3D& yDir,
		const VistaVector3D& vNormVector)
{
	m_Origin      = origin;
	m_XDir        = xDir;
	m_YDir        = yDir;
...
void VistaPlane::CalcPlane(const VistaVector3D &upVector,
							const VistaVector3D &orgPoint)
{
...
/*============================================================================*/
void  VistaPlane::GetEquationRep(float & paramA, float & paramB,
...
{
	float nDist = m_parA * testPnt[Vista::X] +
				  m_parB * testPnt[Vista::Y] +
				  m_parC * testPnt[Vista::Z] +
				  m_parD;
	return nDist;
}
...
/*============================================================================*/
bool VistaPlane::CalcIntersection(const VistaRay& testLine,
								   VistaVector3D& resPnt) const
{
	float lambda;
...
bool VistaPlane::CalcIntersection(const VistaRay& ray, float& lambda) const
{
	...
	if(std::fabs(divisor) < Vista::Epsilon) // the ray and the line are parallel
	{
		//if(fabs(m_parD + m_vNormVector * ray.GetOrigin()) < Vista::Epsilon) // the ray lies in the plane
		//{
		//	lambda = 0.0f;
		//	return true;
		//}
...
{
	if(CalcIntersection(VistaRay(line.GetStartPoint(), line.GetEndPoint() - line.GetStartPoint()), lambda))
	{
...
VistaVector3D VistaPlane::TransformPointOnPlane(
		const VistaVector3D  &givenPnt,
		float rot, float scale, float transX, float transY) const
{
	...
	//                 [   0     0    1 ]   [Vista::Z]   [  Z ]
	float nSinRot = (float)std::sin ( rot * Vista::Pi / 180.0);
	float nCosRot = (float)std::cos ( rot * Vista::Pi / 180.0);
	float tempX = resPnt[Vista::X];
	float tempY = resPnt[Vista::Y];
	resPnt[Vista::X] = (float) ( tempX * nCosRot - tempY * nSinRot );
	resPnt[Vista::Y] = (float) ( tempX * nSinRot + tempY * nCosRot );
	// reconvert to world
	...
	resPnt[Vista::X] =  (float)(resPnt[Vista::X] + transX);
	resPnt[Vista::Y] =  (float)(resPnt[Vista::Y] + transY);
...
/*                                                                            */
/*============================================================================*/
bool VistaPlane::CalcIntersectionPoint(const VistaRay& testLine ,
										VistaVector3D& resolutionPoint) const
{
	...
	else
	{
		nLineFactor    
			=  ( ( CalcDistanceToZero() - 
			   ( m_vNormVector * static_cast<VistaVector3D>( testLine.GetOrigin() ) ) )
			/   ( m_vNormVector * testLine.GetDir() ) );
	}

	// calc the IntersectionPoint and return it
	resolutionPoint 
		= static_cast<VistaVector3D>( testLine.GetDir() ) * (float)nLineFactor + testLine.GetOrigin();
...
int VistaPlane::CalcOrientation(const float px, const float py, const float pz) const
{
	...
	float planeEq = m_vNormVector[0]*(px - m_Origin[0]) +
					m_vNormVector[1]*(py - m_Origin[1]) +
					m_vNormVector[2]*(pz - m_Origin[2]);

	if (std::fabs(planeEq) <= TOLERANCE) return 0; 
	else if (planeEq > TOLERANCE)   return 1;
	else                            return -1; 
}
...
VistaVector3D VistaPolygon::CalcUpVector ()
{
	...
	for (int vtxNum = 0; vtxNum < numPnts; vtxNum ++)
	{
		...
		VistaVector3D    vec2
			(m_vecPolyPnts[vtxNum][Vista::X],
			 m_vecPolyPnts[vtxNum][Vista::Y],
...
// CONSTRUCTOR
VistaPolygonWithHoles::VistaPolygonWithHoles (std::vector<VistaVector3D*> & polyVtx,
...
VistaSplineCurve::VistaSplineCurve(std::vector<VistaVector3D> & points,
									 std::vector<VistaVector3D> & tangents,
									 std::vector<float> & parameters)
  :m_vecPoints(points), m_vecTangents(tangents), m_vecParameters(parameters)
{
	unsigned int i;
	...
	{
		pSegment = new VistaSplineSegment(points[i],points[i+1],tangents[i],
										   tangents[i+1],parameters[i],parameters[i+1]);
...
void VistaSplineCurve::GetSegments (std::vector<VistaCurveSegment*> & segments) const
{
	...
	for(i=0; i< NofSegments(); i++)
	{
		precision = ((VistaSplineSegment *)m_vecCurve[i])->GetPrecision();
		step = 1.0f/precision;
...
VistaTetrahedron::VistaTetrahedron(const float p1[3], const float p2[3],
...
void VistaTetrahedron::SetCoordinates(const float p1[3], const float p2[3],
									   const float p3[3], const float p4[3])
{
...
bool VistaTetrahedron::PtInsideTetrahedron(const float pt[3], float coords[4]) const
{
	...
	if (coords[0] >= epsilon && coords[1] >= epsilon &&
		coords[2] >= epsilon && coords[3] >= epsilon)
		return true;
	else
		return false;
}

float VistaTetrahedron::ComputeVolume(const float p1[3], const float p2[3],
...
: m_nHeight(1.0f), m_nRadius(1.0f)
{
}

VistaEvenCone::VistaEvenCone(float nHeight, float nRadius)
: m_nHeight(nHeight), m_nRadius(nRadius)
{
}
...
bool VistaEvenCone::Intersects(const float pnt[3],
								float &dPerp,
								float &dProj,
								float &dAngle) const
{
	...
	if(  std::fabs(dAngle) <= GetOpeningAngle() )
		return true;
