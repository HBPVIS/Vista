...
VistaConnectionIP::VistaConnectionIP()
{
	...
	SetByteorderSwapFlag((VistaSerializingToolset::GetPlatformEndianess() == VistaSerializingToolset::VST_LITTLEENDIAN));
}
...
VistaConnectionIP::VistaConnectionIP(const VistaProtocol type, const string &sHost, const int iPort, bool bUDPBroadcast)
{
	m_iLingerTime = 0;
#if defined(WIN32)
	m_pWaitEvent = NULL;
	...
	if(m_pSocket)
	{
		...
		if(GetProtocol()== CT_TCP && bBlockingConnect)
		{
			if(m_pSocket->GetIsConnected())
				return false; // we could check iff current connection equals incoming args and return true then, but we do not ;)
		...
		if(adr.GetIsValid())
		{
			...
			{
				m_pSocket->SetIsBlocking(false); // set blocking state to user settings, independant of connection state
...
int VistaConnectionIP::Receive (void *buffer, const int length, int iTimeout  )
{
	if( iTimeout == 0 )
	{
		...
		if(GetProtocol() == CT_TCP && GetIsBlocking())
		{
			...
			{
				iCurrentNum = m_pSocket->ReceiveRaw(pTempBuffer, length);
				if(iCurrentNum <= 0)
				{
...
HANDLE VistaConnectionIP::GetConnectionWaitForDescriptor()
{
	...
	if(m_pWaitEvent == NULL)
		m_pWaitEvent = new VistaThreadEvent(false);
