...
/*                                                                            */
/*============================================================================*/
// $Id: VistaConnectionNamedPipe.cpp 29146 2012-02-08 15:13:47Z dr165799 $
...
namespace
{
	...
	{
		int nFile = open( sName.c_str(), O_WRONLY );
		
		if( nFile < 0 )
		{
			vstr::warnp() << "Opening Named Pipe [" 
								<< sName
	...
	int OpenFIFORead( const std::string& sName, int m_nMaxWait = 10000 )
	{
		...
		for( ;; )
		{
			nFile = open( sName.c_str(), O_RDONLY );
			if( nFile >= 0 )
				break;
			...
			if( oTimer.GetLifeTime() < nMaxWaitSecs )
			{
				vstr::warnp() << "Opening named pipe [" 
								<< sName
								<< "] for reading failed with error [" << errno
								<< "] (" << strerror( errno ) << ")" << std::endl;
				return -1;
			}
		}
	...
	bool CreateFifo( const std::string& sName )
	{
		...
		vstr::warnp() << "Creating named pipe [" 
					<< sName
...
/*============================================================================*/
VistaConnectionNamedPipe::VistaConnectionNamedPipe( const std::string& sPipeName, const bool bIsServer )
...
, m_bIsServer( bIsServer )
{
	...
	if( m_bIsServer )
	{
		...
		m_oPipe = CreateNamedPipe( sActualName.c_str(), PIPE_ACCESS_DUPLEX, PIPE_TYPE_BYTE | PIPE_READMODE_BYTE | PIPE_WAIT, 1, 512, 512, 60000, NULL );
...
bool VistaConnectionNamedPipe::Open()
{
	...
	else
	{
		...
		m_oPipe = CreateFile( sActualName.c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL );
		if( m_oPipe == INVALID_HANDLE_VALUE )
		{
			vstr::warnp() << "[VistaConnectionNamedPipe]: Could not create event ["
						<< m_sPipeName << "] - Error: " << GetErrorText() << std::endl;
...
int VistaConnectionNamedPipe::Receive( void* pBuffer, const int nLength, int nTimeout )
{
	...
	int nRet = read( m_nReadFifo, pBuffer, nLength );	
	if( nRet < 0 )
	{
...
#if !defined(WIN32)
	int nRet = write( m_nWriteFifo, pBuffer, nLength );
	if( nRet != nLength )
	{
...
unsigned long VistaConnectionNamedPipe::PendingDataSize() const
{
	...
	{
		vstr::warnp() << "VistaConnectionNamedPipe::PendingDataSize() -- PeekNamedPipe failed with Error "
...
unsigned long VistaConnectionNamedPipe::WaitForIncomingData( int nTimeout )
{
	...
	FD_SET( int(m_nReadFifo), &oReadSet );
	FD_SET( int(m_nReadFifo), &oExceptionSet );
	int iRet = select( int(m_nReadFifo) + 1, &oReadSet, NULL, &oExceptionSet, pTime );
	if( iRet == 0 )
	{
		// timeout
		return ~0;
	...
	if( FD_ISSET( int(m_nReadFifo), &oExceptionSet ) )
	{
		vstr::warnp() << "VistaConnectionNamedPipe::WaitForIncomingData() -- socket is in exception array" << std::endl;
#ifdef VISTA_IPC_USE_EXCEPTIONS
	...
	{
		vstr::warnp() << "VistaConnectionNamedPipe::WaitForIncomingData() -- WaitForSingleObject failed with Error "
					<< GetLastError() << std::endl;
#ifdef VISTA_IPC_USE_EXCEPTIONS
		VISTA_THROW("IVistaSocket::WaitForIncomingData -- Exception", 0x00000104)
#endif
