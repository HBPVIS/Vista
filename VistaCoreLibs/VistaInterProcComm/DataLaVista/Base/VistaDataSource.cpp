...
/*                                                                            */
/*============================================================================*/
// $Id: VistaDataSource.cpp 21315 2011-05-16 13:47:39Z dr165799 $
...
DLVistaDataSource::~DLVistaDataSource()
{
	printf("DLVistaDataSource::~DLVistaDataSource() -- %d drop for %d produced packets (%d packets in queue: ratio [%f]).\n", 
		m_iDropCount, m_iPackageCount, 
		(*m_pPacketQueue).GetPacketCount(), 
		m_iPackageCount ? ((float)m_iDropCount)/((float)m_iPackageCount) : 0.0f);
...
IDLVistaDataPacket *DLVistaDataSource::ProducePacket()
{
	if(HasPacket())
	{
		...
		else
		{
			++m_iDropCount;
#ifdef DEBUG
			printf("DLVistaDataSource::ProducePacket() -- Data-Drop! Data-ready, but no packet to send!\n");
#endif
...
bool DLVistaDataSource::AcceptDataPacket(IDLVistaDataPacket *, IDLVistaPipeComponent *pComp, bool bBlock )
{
	return false;
};
...
bool DLVistaDataSource::RecycleDataPacket(IDLVistaDataPacket *pPacket, IDLVistaPipeComponent *pComp, bool bBlock)
{
