...
class VISTAINTERPROCCOMMAPI VistaThreadPoolThread : public VistaThread
{
public:
...
: m_pThreadPool(pPool), m_id(iId), m_fProtOut(NULL)
{
}
...
void VistaThreadPoolThread::ThreadBody()
{
	for(;;)
	{
	  ...
	  //      printf("%d: ... thread exit\n", m_id);
		...
		PutMsg(buffer);
#endif
		//PUTMSG("thread=%s %ld -- TP @%x has size = %d\n", (pTest, GetThreadIdentity(), m_pThreadPool, m_pThreadPool->m_vWorkPool.size()));
		...
		{
			if(m_pThreadPool->RemoveDoneJob (pWork->GetJobId()) != pWork)
			{
...
void IVistaThreadPoolWorkInstance::SetJobPriority(VistaPriority &oPrio)
{
	m_oPrio = oPrio;
	if(m_pProcessingThread)
		m_pProcessingThread->SetPriority(oPrio); // mirror to thread
}
...
VistaThreadPool::VistaThreadPool(int iNumMaxThreads, int iNumMaxTasks,
								   const string &sThreadNamePrefix)
: m_bWorkStarted(false), m_bQueueClosed(false), 
  m_bShutdown(false), m_nMaxThreads(iNumMaxThreads),
  m_nTotalNumberOfJobs(0), m_nTotalNumberOfProcessedJobs(0), 
...
int VistaThreadPool::AddWork(IVistaThreadPoolWorkInstance *pWork)
{
	...
	deque<IVistaThreadPoolWorkInstance *>::const_iterator cit = std::find(m_vWorkPool.begin(),
...
{
	m_pQueueLock->Lock();
	deque<IVistaThreadPoolWorkInstance *>::iterator
		it = find(m_vWorkPool.begin(), m_vWorkPool.end(), pWork);
