...
/*                                                                            */
/*============================================================================*/
// $Id: VistaPosixProcessEventImp.cpp 29146 2012-02-08 15:13:47Z dr165799 $
...
, m_nFifo( -1 )
{
	...
	nDelta.tv_usec = 0;
	nDelta.tv_sec = 0;
	while( select( 1, m_oReadSet, NULL, NULL, &nDelta ) > 0 )
	{
		read( m_nFifo, &nDummy, sizeof(char) );
	}
}
...
bool VistaPosixProcessEventImp::SignalEvent()
{
	...
	char nDummy = 0;
	write( m_nFifo, &nDummy, sizeof(char) );
	return true;
}
...
bool VistaPosixProcessEventImp::WaitForEvent( int iBlockTime )
{
	...
	int nRet = select( 1, m_oReadSet, NULL, NULL, &nDelta );
	if( nRet == 0 ) // timeout
		return false;
	else if( nRet == -1 )
	{
		vstr::warnp() << "[VistaPosixProcessEventImp]: "
					<< "Error while waiting for fifo: " 
					<< strerror( errno ) << std::endl;
		return false;
	}

	char nTarget;
	if( read( m_nFifo, &nTarget, sizeof(char) ) == -1 )
	{
		vstr::warnp() << "[VistaPosixProcessEventImp]: "
...
bool VistaPosixProcessEventImp::WaitForEvent( bool bBlock )
{
	...
	if( bBlock == false )
	{
		...
		if( select( 1, m_oReadSet, NULL, NULL, &nDelta ) == 0 )
			return false;
	}
