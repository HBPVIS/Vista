...
/*                                                                            */
/*============================================================================*/
// $Id: VistaBroadcastClusterBarrier.cpp 29293 2012-02-14 17:47:59Z dr165799 $
...
VistaClusterLeaderBarrierIPBase::Follower::Follower( const std::string& sName,
													VistaConnectionIP* pConn,
													...
													const bool bManageConnectionDeletion )
{
	...
	if( pConnection == NULL || pConnection->GetIsConnected() == false )
	{
		...
		m_vecFollowers.push_back( new Follower( sName, NULL, bManageConnectionDeletion, m_nLastChangedFollower ) );
	}
	else
	{
		Follower* pFollower = new Follower( sName, pConnection, bManageConnectionDeletion, m_nLastChangedFollower );
			...
			itFollower != m_vecAliveFollowers.end(); ++itFollower )
	{
		...
		try
		{
			...
			if( nRes == 0 )
			{
				...
				if( ++(*itFollower)->m_nSyncFailures >= 10 )
				{
					vstr::errp() << "[" << GetBarrierType() << "]: "
								<< "10 consecutive sync failures for [" << (*itFollower)->m_sName
			...
			if( nAckCounter != m_nBarrierWaitCount )
			{
				...
				{
					vstr::warnp() << "[" << GetBarrierType() << "]: "
								<< "Follower [" << (*itFollower)->m_sName
								<< "] reported count [" << nAckCounter << "], but expected ["
								<< m_nBarrierWaitCount << "] " << std::endl;
				}
				if( ++(*itFollower)->m_nSyncFailures >= 10 )
				{
					vstr::errp() << "[" << GetBarrierType() << "]: "
...
std::vector<VistaClusterLeaderBarrierIPBase::Follower*>& VistaClusterLeaderBarrierIPBase::GetFollowersRef()
{
...
, m_nBarrierWaitCount( 0 )
{
	if( m_pLeaderConn == NULL || m_pLeaderConn->GetIsConnected() == false )
	{
		vstr::errp() << "[VistaClusterFollowerBarrierIPBase]: "
					<< "Received leader connection is not connected - syncing will not work" << std::endl;
