...
, m_nCollectCount( 0 )
{
	m_oDeSer.SetByteorderSwapFlag( false ); // only slaves are responsible for swapping
}
...
int VistaTCPIPClusterLeaderDataCollect::AddFollower( const std::string& sName,
...
bool VistaTCPIPClusterLeaderDataCollect::CollectTime( const VistaType::systemtime nOwnTime,
...
bool VistaTCPIPClusterLeaderDataCollect::CollectData( const VistaPropertyList& oList,
...
bool VistaTCPIPClusterLeaderDataCollect::ReceivePartFromFollower( int nExpectedType,
																 Follower* pFollower,
																 const int nTimeout )
{
	try
	...
	catch( VistaExceptionBase& )
	{
		vstr::warnp() << "[VistaTCPIPClusterLeaderDataCollect]: "
					<< "Exception while reading data from follower "
					<< pFollower->m_sName << "] - removing it" << std::endl;
	...
	if( pLeaderConn->GetIsConnected() == false )
	{
		if( GetIsVerbose() )
		{
			vstr::errp() << "[VistaTCPIPClusterFollowerDataCollect]: "
						<< "Received leader connection is not connected - Collecting will not work" << std::endl;
		}
		Cleanup();
	}
...
bool VistaTCPIPClusterFollowerDataCollect::CollectTime( const VistaType::systemtime nOwnTime,
							std::vector<VistaType::systemtime>& vecCollected )
{
...
bool VistaTCPIPClusterFollowerDataCollect::CollectData( const VistaPropertyList& oList,
						std::vector<VistaPropertyList>& vecCollected )
{
