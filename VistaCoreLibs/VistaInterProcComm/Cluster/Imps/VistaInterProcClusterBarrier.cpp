...
/*                                                                            */
/*============================================================================*/
// $Id: VistaInterProcClusterBarrier.cpp 29428 2012-03-02 13:46:32Z dr165799 $
...
VistaInterProcClusterLeaderBarrier::VistaInterProcClusterLeaderBarrier( 
									IVistaClusterBarrier* pDependentBarrier,
									const bool bVerbose )
...
{	
	...
	for( std::vector<std::pair<VistaProcessEventReceiver*, int> >::iterator itEvent = m_vecReadyEvents.begin();
...
{	
	VistaProcessEventReceiver* pReadyEvent = new VistaProcessEventReceiver( sInterProcName + "_READY", 1000 );
...
bool VistaInterProcClusterLeaderBarrier::BarrierWait( int iTimeOut )
{
	...
	for( std::vector<std::pair<VistaProcessEventReceiver*, int> >::iterator itEvent
			= m_vecReadyEvents.begin(); itEvent != m_vecReadyEvents.end();
			++itEvent )
	{
		bool nRet;
		if( iTimeOut == 0 )
	...
	// send go to all
	for( std::vector<VistaProcessEventSignaller*>::iterator itEvent
			= m_vecGoEvents.begin(); itEvent != m_vecGoEvents.end();
			++itEvent )
	{
		(*itEvent)->SignalEvent();
	}
...
bool VistaInterProcClusterFollowerBarrier::BarrierWait( int iTimeOut )
{
	...
	if( nRet == false )
	{
		...
		{
			vstr::errp() << "[" << GetBarrierType() << "]: 10 concecutive timeouts - aborting" << std::endl;
			delete m_pReadyEvent;
			m_pReadyEvent = NULL;
			delete m_pGoEvent;
			m_pGoEvent = NULL;
		...
		{
			vstr::warnp() << "[" << GetBarrierType() << "]: timeout while waiting for go event" << std::endl;
		}
	}
	else
		m_nTimeoutCount = 0;
