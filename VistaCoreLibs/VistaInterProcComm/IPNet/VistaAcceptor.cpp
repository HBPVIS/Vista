...
{
	m_pSyncEvent = new VistaThreadEvent(true);
	m_bConnected = false;
	m_pServer = NULL;
...
{
	//printf("VistaAcceptWork::DefinedThreadWork()\n");
	if(!m_pServer)
	{
	...
	VistaThreadEvent sockEvent(true); // on UNIX, create Posix-Event-Type
	...
	printf("VistaAcceptWork::DefinedThreadWork() -- WAITING FOR CONNECT (OR ABORT)\n");
	int iRet = select((iSckId>iSyncId ? iSckId+1 : iSyncId+1), &myRdSocks, NULL, &myExSocks, NULL);
