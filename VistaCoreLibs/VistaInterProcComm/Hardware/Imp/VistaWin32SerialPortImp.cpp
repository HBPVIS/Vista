...
static string _build_dcb_string(const VistaWin32SerialPortImp &con )
{
	...
	{
		sprintf((char*)sRet.data(), "%d,%c,%d,%d", con.GetSpeed(), (con.GetParity() == VistaSerialPortStateDesc::CSPA_EVEN ? 'n' : 'y'),
										con.GetDataBits(), con.GetStopBits());
	}
	else
	{
		sprintf((char*)sRet.data(), "%d,%c,%d,1.5", con.GetSpeed(), (con.GetParity() == VistaSerialPortStateDesc::CSPA_EVEN ? 'n' : 'y'),
										con.GetDataBits());
	}
...
bool VistaWin32SerialPortImp::SetHardwareState()
{
	...
	else
	{
		...
		m_myDcb.fInX = m_myDcb.fOutX = 
			m_myDcb.fOutxDsrFlow =
			m_myDcb.fOutxCtsFlow = FALSE;
...
bool VistaWin32SerialPortImp::GetHardwareState()
{
	...
	if( (m_myDcb.fOutxCtsFlow == TRUE) && (m_myDcb.fOutxDsrFlow == FALSE) && (m_myDcb.fRtsControl == RTS_CONTROL_HANDSHAKE))
...
bool VistaWin32SerialPortImp::OpenSerialPort()
{
	...
	if((m_hanPort = CreateFile(GetPortName().c_str(), 
...
int  VistaWin32SerialPortImp::Receive(void *buffer, const int length, int iTimeout ) 
{
	...
	else
	{
		...
		DWORD retval = 0;
		if((retval = ::WaitForSingleObject(HANDLE(m_hOverlap.hEvent), DWORD(iTimeout))) == WAIT_OBJECT_0)
...
bool VistaWin32SerialPortImp::SetBlockingMode ( unsigned long inReadInterval, unsigned long inReadMultiplyer, unsigned long inReadConstant )
{
