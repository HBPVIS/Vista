...
/*                                                                            */
/*============================================================================*/
// $Id: VistaPosixSerialPortImp.cpp 21315 2011-05-16 13:47:39Z dr165799 $
...
bool VistaPosixSerialPortImp::OpenSerialPort()
{
	...
	if((m_iFileHandle = open(GetPortName().c_str(), O_RDWR|O_NOCTTY)) != -1)
	{
		SetIsOpen(true);
...
int  VistaPosixSerialPortImp::Send(const void *buffer, const int length) 
{
	...
	if ( (charsWritten = write( m_iFileHandle, buffer, length ) ) < 0)
	{
		// this failed
		return -1;
	}
...
unsigned long VistaPosixSerialPortImp::WaitForIncomingData(int timeout)
{
	...
	int iRet = select(m_iFileHandle+1, &fdset, NULL, NULL, ((tv.tv_sec!=0 || tv.tv_usec!=0) ? &tv : NULL));
	if(iRet == 0)
	{
		//printf("timeout?\n");
		return ~0;
