...
{
public:
	SkyboxData(VistaOpenSGSkybox *pParent,
	...
	struct _sHlp
	{
		_sHlp(VistaViewport *pVp,
			OSG::BackgroundPtr p,
			OSG::BackgroundPtr pR)
			: m_pVp(pVp),
			m_pOldBackground(p),
		    m_pOldRightBack(pR) {}

		bool operator==(const _sHlp &other) const { return other.m_pVp == m_pVp; }
		bool operator< (const _sHlp &other) const { return other.m_pVp <  m_pVp; }

		VistaViewport *m_pVp;
		OSG::BackgroundPtr m_pOldBackground,
			               m_pOldRightBack;
	};
...
/*============================================================================*/
VistaOpenSGSkybox::VistaOpenSGSkybox(const std::string &strTop,
									   const std::string &strBottom,
									   const std::string &strLeft,
									   const std::string &strRight,
									   const std::string &strFront,
...
: m_pData( NULL )
{
	m_pData = new SkyboxData( this, strTop, strBottom,
			                         strLeft, strRight,
			                         strFront, strBack );
}
...
{
	return (m_pData->m_liVp.find(SkyboxData::_sHlp(pViewport, OSG::NullFC, OSG::NullFC)) != m_pData->m_liVp.end());
}
