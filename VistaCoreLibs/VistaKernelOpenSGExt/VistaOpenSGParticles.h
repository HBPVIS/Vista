...
public:
	...
	/**
	* @BRIEF Returns the beginning address of Secondary Particle Position field - [Particle_Number][3]
	*/ 
	...
	*/ 
	void SetShaderPrograms(const std::string &VertexShaderProgram, const std::string &FragmentShaderProgram) const;
...
private:
	...
	*/
	void SetParticleShader() const;
	/**
	* @BRIEF Sets the Normal for our Particle System (0,1,0)
	*/ 
	void SetNormal() const;
	/**
	* @BRIEF Setting the Material for the Particles. ONLY tested for use with Shader Quads/Stripes!
	*/ 
	void SetSimpleTexturedMaterial(const std::string &FileName) const;
	/**
	...
	*/ 
	...
	/**
	* @BRIEF Only use this call except you want to go into detail! Creating ShaderQuads (semi-billboarding) with direction (SECOND POSITION ARRAY!!!)
	*/
