		          ...
		          VistaQuaternion &qResultingTorque)
{
	...
	const _sPlane *pPlane = reinterpret_cast<const _sPlane*>( (&( *pM ).m_vecMeasures[0]) );
	...
	if( fabs(nDistance) < 1.e-4f) // make a small tolerance around for points within the plane
	{
		// point on plane
...
{
	microtime dTimeStamp = VistaTimeUtils::GetStandardTimer().GetMicroTime();
	...
	_sPlane *pPlane = reinterpret_cast<_sPlane*>( (&( *pM ).m_vecMeasures[0]) );
