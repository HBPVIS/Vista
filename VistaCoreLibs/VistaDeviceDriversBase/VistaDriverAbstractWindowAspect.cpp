...
bool VistaDriverAbstractWindowAspect::AttachToWindow(CWindowHandle *pWindow)
{
	if(!m_pTouchSeq)
		return false;
...
bool VistaDriverAbstractWindowAspect::DetachFromWindow(CWindowHandle *pWindow)
{
	if(!m_pTouchSeq)
		return false;
...
std::list<CWindowHandle*> VistaDriverAbstractWindowAspect::GetWindowList() const
{
	if(m_pTouchSeq)
