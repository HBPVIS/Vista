...
VistaDriverConnectionAspect::~VistaDriverConnectionAspect()
{
	for(CONVEC::iterator it = m_vecConnections.begin();
...
{
	for(CONVEC::const_iterator cit = m_vecConnections.begin();
		cit != m_vecConnections.end(); ++cit)
	{
...
unsigned int VistaDriverConnectionAspect::GetIndexForRole(const std::string &sToken) const
{
	unsigned int n=0;
	for(CONVEC::const_iterator cit = m_vecConnections.begin();
		cit != m_vecConnections.end(); ++cit, ++n)
	{
...
void VistaDriverConnectionAspect::SetConnection(unsigned int nIndex,
												 ...
												 bool bForce)
{
	...
	if(m_vecConnections[nIndex] == NULL)
		...
		m_vecConnections[nIndex] = new ConnectionHlp(this, pConnection, NULL, NULL, bCanBeReplaced);
	else
	{
		...
		if(m_vecConnections[nIndex]->m_pConnection)
		{
			...
			// below
			if(m_vecConnections[nIndex]->m_bCanBeReplaced || bForce)
...
bool VistaDriverConnectionAspect::SetAttachSequence(unsigned int nConIdx,
													 IVistaConnectionSequence *pSequence)
{
	// create enough storage, even when inserting NULLs
...
VistaDriverConnectionAspect::IVistaConnectionSequence *VistaDriverConnectionAspect::GetAttachSequence(unsigned int nConIdx) const
{
	if(nConIdx >= m_vecConnections.size())
		return NULL;
...
bool VistaDriverConnectionAspect::SetDetachSequence(unsigned int nConIdx,
													 IVistaConnectionSequence *pSequence)
{
	// create enough storage, even when inserting NULLs
	if(nConIdx >= m_vecConnections.size())
...
VistaDriverConnectionAspect::IVistaConnectionSequence *VistaDriverConnectionAspect::GetDetachSequence(unsigned int nConIdx) const
{
	if(nConIdx >= m_vecConnections.size())
		return NULL;
...
bool VistaDriverConnectionAspect::SendCommand(unsigned int nIndex,
										const void *pcCmd, unsigned int unLength,
										unsigned int nPostSendDelay)
{
	if(nIndex >= m_vecConnections.size())
		return false;
...
int  VistaDriverConnectionAspect::GetCommand(unsigned int nIndex,
											  void *pcBuffer,
											  unsigned int nMaxLength,
											  int nTimeout, bool *pTOMark)
{
	if(nIndex >= m_vecConnections.size())
		return false;
...
int  VistaDriverConnectionAspect::GetTerminatedCommand(unsigned int nIndex,
														void *pvBuffer,
														unsigned int nMaxLength,
														char cMark,
														int nTimeout,
...
VistaDriverConnectionAspect::ConnectionHlp *VistaDriverConnectionAspect::GetHlpByConnection(VistaConnection *pCon) const
{
	for(CONVEC::const_iterator cit = m_vecConnections.begin();
		cit != m_vecConnections.end(); ++cit)
	{
