...
public:
	VistaUpdateThreadWork(IVistaExplicitCallbackInterface *pCb)
		...
		m_pJobDone(new VistaThreadEvent(true)),
		m_pCallback(pCb),
		m_nMode(0)
	{
...
public:
	...
	virtual ~UpdateThread()
	{
		...
		// updater
		for(std::vector<UPDIT>::iterator it = m_vecUpdated.begin(); 
			it != m_vecUpdated.end(); ++it) // while iterating, increase ticket...
		{
			delete (*it).second;
		}
							 ...
							 IVistaExplicitCallbackInterface *pInt)
	{
		...
		if(m_mp.AddMultiplexPoint(pCon->GetConnectionWaitForDescriptor(),
							   ...
							   VistaIOMultiplexer::eIODir(VistaIOMultiplexer::MP_IOIN
							  | VistaIOMultiplexer::MP_IOERR))) // add for all access
		{
			return true;
		}
		else
	...
	IVistaExplicitCallbackInterface *RemConnectionUpdate(VistaConnection *pCon)
	{
		...
		for(std::vector<UPDIT>::iterator it = m_vecUpdated.begin(); 
			it != m_vecUpdated.end(); ++it, ++i) // while iterating, increase ticket...
		{
			if( (*it).first == pCon)
			{
	...
	{
		for(std::vector<UPDIT>::const_iterator it = m_vecUpdated.begin(); 
			it != m_vecUpdated.end(); ++it)
		{
			if( (*it).first == pCon)
			{
...
//  				printf("adding event  job wait\n");
				...
				m_mp.AddMultiplexPoint(up.second->m_pJobDone->GetEventWaitHandle(),
					nIdx, VistaIOMultiplexer::eIODir(VistaIOMultiplexer::MP_IOIN
...
//  				printf("DONE\n") ;
			...
			else
			{
				...
				b = m_mp.AddMultiplexPoint(HANDLE(up.first->GetConnectionWaitForDescriptor()),
					nIdx, VistaIOMultiplexer::eIODir(VistaIOMultiplexer::MP_IOIN
					| VistaIOMultiplexer::MP_IOERR));
...
private:
	...
	typedef std::pair<VistaConnection*, 
					  VistaUpdateThreadWork*> UPDIT;
...
/* IMPLEMENTATION                                                             */
/*============================================================================*/
bool VistaConnectionUpdater::AddConnectionUpdate(VistaConnection *pCon,
						 IVistaExplicitCallbackInterface *pInt)
{
...
IVistaExplicitCallbackInterface *VistaConnectionUpdater::RemConnectionUpdate(VistaConnection *pCon)
{
	return (*m_pUpdateThread).RemConnectionUpdate(pCon);
}

IVistaExplicitCallbackInterface *VistaConnectionUpdater::GetConnectionUpdate(VistaConnection *pCon) const
{
