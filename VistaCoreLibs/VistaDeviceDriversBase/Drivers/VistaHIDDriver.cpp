...
bool VistaHIDDriver::DoSensorUpdate(microtime dTs)
{
...
*/
	...
	case WAIT_OBJECT_0:
	{
		...
		if (rv)
		{
			...
			for (size_t i = 0; i < numBytes / 7; i++) {
				...
				sshort16 *bufptr = reinterpret_cast<sshort16*>(&report[1]);
	...
	case WAIT_FAILED:
	{
		fprintf(stderr, "[HIDDriver] WaitForSingleObject returned error status (GLE %u)\n", GetLastError());
		ResetEvent(m_hReadEvent);
	}
	case WAIT_ABANDONED:
	{
