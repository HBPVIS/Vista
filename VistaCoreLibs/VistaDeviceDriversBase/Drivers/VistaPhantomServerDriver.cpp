...
public:
	...
	virtual bool operator()(VistaConnection *pCon)
	{
		...
		try
		{
			...
			vstr::outf() << "Found phantom device with vendor: [" << vendor << "], device: [" << deviceType << "]" << std::endl;
		}
...
bool VistaPhantomServerDriver::DoSensorUpdate(VistaType::microtime dTs)
{
	try
	{
		...
		if(pIncoming && pIncoming->GetIsOpen())
		{
			...
			if(nReadSize > 0)
			{
				std::vector<VistaType::ubyte8> oMsg(sizeof(_sPhantomMeasure));
				...
				{
					nDataRead = pIncoming->ReadRawBuffer((void*)&oMsg[0], sizeof(_sPhantomMeasure));
					if(nDataRead < 0)
					{
						// TROUBLE!
				...
				deSer.SetBuffer( &oMsg[0], sizeof(_sPhantomMeasure) );
		...
		if(pControl && pControl->GetIsOpen())
		{
			...
			if((nDataSize=pControl->PendingDataSize())>0)
			{
				...
				for (unsigned int i=0; i<nDataSize; i+=nDataRead)
				{
					...
					if( nDataRead <= 0)
					{
						vstr::errf() << "[VistaPhantomServerDriver::DoSensorUpdate]: Read error" << std::endl;
						pControl->Close();
						return false;
					}
				}
				...
				case 'E': // error flag sent
					{
						...
						vstr::errf() << "[VistaPhantomServer] Error: " << errorId << " " << sErrorMsg << std::endl;
