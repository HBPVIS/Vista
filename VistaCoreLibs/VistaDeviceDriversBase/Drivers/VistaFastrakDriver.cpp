	...
	virtual bool CmdInitSequence()
	{
		...
		if(pCon->Send( data, 3 ) == 3)
		{
			...
			if(pCon->Send( data, 2 ) == 2)
			{
				std::string strAnswer;
				pCon->ReadDelimitedString( strAnswer, 0x0A ); // liberty resets
				//vstr::debug() << "answer : " << strAnswer << std::endl;
				pCon->ReadDelimitedString( strAnswer, 0x0A ); // gives an endline
				//vstr::debug() << "answer : " << strAnswer << std::endl;
				pCon->ReadDelimitedString( strAnswer, 0x0A ); // says 'liberty restarted'
				//vstr::debug() << "answer : " << strAnswer << std::endl;
				...
				{
					vstr::outf() << "VistaFastrakDriver switched to 120Hz update rate" << std::endl;
				}

				if(CmdEnableMetricMode())
				{
					vstr::outf() << "VistaFastrakDriver switched to metric mode" << std::endl;
				}
				...
				vstr::outf() << "[VistaFAstrakDriver]: this device has " << m_lStations << " stations" << std::endl;
				...
				vstr::outf() << "[VistaFAstrakDriver]: this device has " << m_lActiveStations << " active stations" << std::endl;
				...
				//{
				//	std::cout << "Set RefFrame to: a[" << Vista::RadToDeg(euang.a)
				//		      << "] ; e[" << Vista::RadToDeg(euang.b)
				//			  << "] ; r[" << Vista::RadToDeg(euang.c) << "]\n";
				//	CmdSetSourceMountingFrame( Vista::RadToDeg(euang.a),
				//		                       Vista::RadToDeg(euang.b),
				//							   Vista::RadToDeg(euang.c) );
				//}
...
public:
	...
	bool SetProtocol( const _cVersionTag &oTag )
	{
		...
		{
			vstr::errf() << "Protocol [FASTRAK] known, but not supported..." << vstr::endl;
			return false;
		}
		else if(oTag.m_strProtocolName == "LIBERTY")
		{
			vstr::outf() << "Fastrak using the LIBERTY protocol" << std::endl;
		...
		{
			vstr::errf() << "Protocol [LIBERTY] known, but not supported..." << std::endl;
			return false;
		}
...
// ############################################################################
// FASTRAK STATIC CREATION METHODS (DRIVER AND TRANSCODERS)
// ############################################################################
...
class FastrackCreationMethod : public IVistaDriverCreationMethod
{
public:
	virtual IVistaDeviceDriver *operator()()
	{
		return new VistaFastrakDriver;
	}
};
...
class VistaFastrackPosGet : public IVistaMeasureTranscode::CV3Get
{
public:
	VistaFastrackPosGet()
	...
	{
		VistaFastrakDriver::_sFastrakSample *m = (VistaFastrakDriver::_sFastrakSample*)&(*pMeasure).m_vecMeasures[0];
...
class VistaFastrackOriGet : public IVistaMeasureTranscode::CQuatGet
{
public:
	VistaFastrackOriGet()
...
public:
	...
	{
		VistaFastrakDriver::_sFastrakButtonSample *m = (VistaFastrakDriver::_sFastrakButtonSample*)&(*pMeasure).m_vecMeasures[0];
		bRet = m->m_bBtPress;
		return true;
	}
};
...
: IVistaDeviceDriver(),
  ...
  m_pSensorMap( new VistaDriverSensorMappingAspect),
  m_pCommandSet( NULL ),
  m_pProtocol(NULL),
  m_pInfoAspect(NULL),
  m_pRefFrame(NULL)
{
	...
	m_nWandType = m_pSensorMap->RegisterType( "WAND", sizeof(_sFastrakButtonSample), 120, new  VistaFastrakWandFactory);
	m_nBodyType = m_pSensorMap->RegisterType( "BODY", sizeof(_sFastrakSample), 120, new VistaFastrakBodyFactory );
...
VistaFastrakDriver::~VistaFastrakDriver()
{
	UnregisterAspect(m_pRefFrame, false);
	delete m_pRefFrame;

	UnregisterAspect( m_pInfoAspect, false );
	delete m_pInfoAspect;

	UnregisterAspect( m_pConnection, false );
	delete m_pConnection;
	UnregisterAspect( m_pSensorMap, false );
	delete m_pSensorMap;
...
/* IMPLEMENTATION                                                             */
/*============================================================================*/
unsigned int VistaFastrakDriver::GetSensorMeasureSize() const
{
	return sizeof(_sFastrakSample);
}
	...
	while(n < 3*GetCommandSet()->CmdGetActiveStations())
	{
				...
				&nSensor, &fPx, &fPy, &fPz, &fOw, &fOx, &fOy, &fOz, &nButtonFlag, &nTimeStamp, &nFrameCount, &nStylus, &nDistortionLevel )) == 13 )
		{
			...
			if(nStylus == 0) // simple body
			{
				...
				_sFastrakButtonSample *pMeasure = (_sFastrakButtonSample*)&(*pM).m_vecMeasures[0];
...
bool VistaFastrakDriver::Connect()
{
	VistaConnection *pCon = m_pConnection->GetConnection();
	if(pCon == NULL)
	...
	//vstr::outf() << pCon->PendingDataSize() << std::endl;
	...
	try
	{
		...
		if(bRet)
		{
			...
			{
				props.SetValue( VistaAspectsConversionStuff::ConvertToString( n++ ), *cit );
			}
