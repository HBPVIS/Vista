...
char VistaSpaceMouseDriver::SCodeTable[16] =
{
	'0','A','B','3',
	'D','5','6','G',
	'H','9',':','K',
...
public:
	...
	{
		_sSpaceMouseMeasure *m = (_sSpaceMouseMeasure*)&(*pMeasure).m_vecMeasures[0];
...
class VistaSpacemouseTranscoderFactory : public IVistaMeasureTranscoderFactory
{
public:
	virtual IVistaMeasureTranscode *CreateTranscoder()
	{
		return new VistaSpaceMouseMeasureTranscode;
	}
...
bool VistaSpaceMouseDriver::DoSensorUpdate(microtime dTs)
{
	// we expect a blocking read
	//m_pConnection->SetIsBlocking(0, true);
	...
	// we switch the connection back to blocking (maybe we should
	// move this decision to the workload thread that call the update?)

	//m_pConnection->SetIsBlocking(0, false);
	...
	case 'd':
		{
			...
			m_qOri = VistaQuaternion(
					       VistaEulerAngles(
							Vista::DegToRad(fCoords[3]),
							Vista::DegToRad(fCoords[4]),
							Vista::DegToRad(fCoords[5])) );
			break;
		}
	...
	_sSpaceMouseMeasure *m = (_sSpaceMouseMeasure*)&(*pM).m_vecMeasures[0];
...
bool VistaSpaceMouseDriver::CmdReset(void)
{
	...
	return m_pConnection->SendCommand(0, cResetCommand, (unsigned int)strlen((char*)cResetCommand), 300);
}

bool VistaSpaceMouseDriver::CmdGetDeviceName(std::string &sName)
{
	unsigned char cDeviceCommand[]="\r\rvQ\r";
	bool bResult = m_pConnection->SendCommand(0, cDeviceCommand, (unsigned int)strlen((char*)cDeviceCommand), 1000);
...
{
	int nResult =  DecodeValue(cBuffer[0])*4096
		+ DecodeValue(cBuffer[1])*256
		+ DecodeValue(cBuffer[2])*16
		+ DecodeValue(cBuffer[3])*1   - 32768;
...
unsigned int VistaSpaceMouseDriver::GetSensorMeasureSize() const
{
	return sizeof(_sSpaceMouseMeasure);
}

// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++


class VistaSpaceMouseCreationMethod : public IVistaDriverCreationMethod
{
public:
	virtual IVistaDeviceDriver *operator()()
	{
		return new VistaSpaceMouseDriver;
	}
};
...
IVistaDriverCreationMethod *VistaSpaceMouseDriver::GetDriverFactoryMethod()
{
	if(SpFactory == NULL)
	{
		SpFactory = new VistaSpaceMouseCreationMethod;
		SpFactory->RegisterSensorType("", sizeof(_sSpaceMouseMeasure),
				                      100,
				                      new VistaSpacemouseTranscoderFactory,
				                      VistaSpaceMouseMeasureTranscode::GetTypeString());
	}
