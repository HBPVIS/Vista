...
: IVistaDeviceDriver(sizeof(_sMouseMeasure))
{
...
IVistaMouseDriver::~IVistaMouseDriver()
{
	...
	for(unsigned int n=0; n < GetNumberOfSensors(); ++n)
	{
		...
		VistaMouseDriverMeasureTranscode *pTC = dynamic_cast<VistaMouseDriverMeasureTranscode*>(pSen->GetMeasureTranscode());
		delete pTC;
...
unsigned int IVistaMouseDriver::AddDeviceSensor(VistaDeviceSensor *pSensor)
{
	...
	{
		pSensor->SetMeasureTranscode( new VistaMouseDriverMeasureTranscode );
	}
	return nIdx;
}
...
unsigned int IVistaMouseDriver::GetSensorMeasureSize() const
{
	return sizeof(_sMouseMeasure);
}
