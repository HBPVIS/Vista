...
bool VistaGraphicsManager::Init( IVistaNodeBridge* pNodeBridge,
...
bool VistaGraphicsManager::SetupScene( const std::string& sIniSection,
										const VistaPropertyList& oConfig )
{
	...
	for( std::list<std::string>::const_iterator itLight = liLights.begin();
			itLight != liLights.end(); itLight++)
	{
		vstr::outi() << "Creating light [" << (*itLight) << "]" << std::endl;
		if( oConfig.HasSubList( (*itLight) ) == false )
		{
			vstr::warnp() << "Requested light [" << (*itLight)
				<< "] but no such sectione extists" << std::endl;
			continue;
		}
...
void VistaGraphicsManager::Debug( std::ostream& oOut ) const
{
	...
	oOut << vstr::indent << " [ViGraMan]----------------------------------------------------------" << std::endl;
	oOut << vstr::indent << " [ViGraMan] INFO SCREEN " << std::endl;
	oOut << vstr::indent << " [ViGraMan] VistaSceneGraph\t\t" << ( m_pSceneGraph ? "constructed" : "construction FAILED." ) << std::endl;
	vstr::IndentObject oIndent;
	if( m_pSceneGraph != NULL )
		m_pSceneGraph->Debug( oOut );
	oOut << vstr::indent << " [ViGraMan]	Application is currently running at " << GetFrameRate() << " fps." << std::endl;
	oOut << vstr::indent << " [ViGraMan]----------------------------------------------------------" << std::endl;
}
