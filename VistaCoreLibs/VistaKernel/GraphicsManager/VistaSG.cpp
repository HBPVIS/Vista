...
/*                                                                            */
/*============================================================================*/
// $Id: VistaSG.cpp 21315 2011-05-16 13:47:39Z dr165799 $
...
// ============================================================================
// ============================================================================
bool VistaSG::Init(IVistaNodeBridge* pNodeBridge, 
...
// ============================================================================
// ============================================================================
VistaGroupNode* VistaSG::NewGroupNode(VistaGroupNode* pParent)
{
	if(!m_pNodeBridge)
		return NULL;
...
// ============================================================================
// ============================================================================
VistaSwitchNode* VistaSG::NewSwitchNode(VistaGroupNode* pParent)
{
	if(!m_pNodeBridge)
		return NULL;
...
// ============================================================================
// ============================================================================
VistaLODNode* VistaSG::NewLODNode(VistaGroupNode* pParent)
{
	if(!m_pNodeBridge)
		return NULL;
...
// ============================================================================
// ============================================================================
VistaGeomNode* VistaSG::NewGeomNode(VistaGroupNode* pParent, VistaGeometry* pGeom)
{	
...
VistaTransformNode *VistaSG::NewTransformNode(VistaGroupNode *pParent)
{
	if(!m_pNodeBridge)
		return NULL;
...
// ============================================================================
/*
VistaLightNode* VistaSG::NewLightNode(VistaGroupNode* pParent, VISTA_LIGHTTYPE t)
{
	if(!m_pNodeBridge)
		return NULL;
...
VistaAmbientLight*	VistaSG::NewAmbientLight (VistaGroupNode* pParent)
{
	if(!m_pNodeBridge)
		return NULL;
...
VistaDirectionalLight* VistaSG::NewDirectionalLight (VistaGroupNode* pParent)
{
	if(!m_pNodeBridge)
		return NULL;
...
VistaPointLight* VistaSG::NewPointLight (VistaGroupNode* pParent)
{
	if(!m_pNodeBridge)
		return NULL;
...
VistaSpotLight* VistaSG::NewSpotLight (VistaGroupNode* pParent)
{
	if(!m_pNodeBridge)
		return NULL;
...
VistaOpenGLNode* VistaSG::NewOpenGLNode(VistaGroupNode* pParent, IVistaOpenGLDraw* pDI)
{
	//New implementation specific representation of lightnode
...
VistaExtensionNode *VistaSG::NewExtensionNode(VistaGroupNode *pParent, IVistaExplicitCallbackInterface *pExt)
{
...
VistaGeometry* VistaSG::NewIndexedGeometry(
...
// ============================================================================
// ============================================================================
bool VistaSG::DeleteLeaf(VistaLeafNode* pLeafNode)
{
	if(!pLeafNode)
		return false;
...
// ============================================================================
// ============================================================================
bool VistaSG::DeleteGroupNode(VistaGroupNode* pGroupNode, bool bMoveChildren)
{
...
// ============================================================================
// ============================================================================
bool VistaSG::DeleteSubTree(VistaGroupNode* pSubTreeRoot)
{
	if(!pSubTreeRoot)
		return false;
	stack<VistaGroupNode*> NodeStack;
...
// ============================================================================
// ============================================================================
bool VistaSG::DeleteGeometry(VistaGeometry* pGeom)
{
	delete pGeom;
	return true;
}
...
// ============================================================================
// ============================================================================
void VistaSG::Debug(std::ostream& out) const
{
	VistaTransformMatrix matrix;
	m_pRoot->GetTransform(matrix);;
	out << " [ViSG] ----------------------------------------------------------------" << endl;
	out << " [ViSG] INFO SCREEN " << endl;
	out << " [ViSG] Current ROOT_NODE transform is: " << endl;
	out << matrix;
	out << " [ViSG] Current SG layout is : " << endl;
...
// ============================================================================
// ============================================================================
VistaGroupNode* VistaSG::GetRealRoot() const
{
	return m_pRoot;
}

VistaGroupNode* VistaSG::GetRoot() const
{
	return m_pModelRoot;
}
// ============================================================================
// ============================================================================
IVistaNode* VistaSG::GetNode(const std::string &strName,
...
// ============================================================================
// ============================================================================
VistaGroupNode* VistaSG::LoadSubTree(const std::string &strFileName, 
									   VistaSG::eOptFlags eOpts,
									   float fScale)
{
	IVistaNode* pLoadedNode = m_pNodeBridge->LoadNode(strFileName, fScale, IVistaNodeBridge::eOptFlags(eOpts));
...
// ============================================================================
// ============================================================================
bool VistaSG::SaveSubTree(std::string strFileName, VistaGroupNode* pNode)
{
	if(!pNode)
		return false;
...
// ============================================================================
// ============================================================================
VistaGeomNode* VistaSG::LoadGeomNode(const std::string &strFileName, 
									   VistaSG::eOptFlags eOpts,
									   float fScale)
{
	IVistaNode* pLoadedNode = m_pNodeBridge->LoadNode(strFileName, fScale, IVistaNodeBridge::eOptFlags(eOpts));
...
IVistaNode * VistaSG::LoadNode( const std::string &sFileName, 
								 VistaSG::eOptFlags eOpts,
				float fScale)
{
	return m_pNodeBridge->LoadNode( sFileName , fScale, IVistaNodeBridge::eOptFlags(eOpts) );
}

bool VistaSG::ApplyOptimizationToNode( IVistaNode *pNode,
										eOptFlags eFlags )
{
	return m_pNodeBridge->ApplyOptimizationToNode(pNode, IVistaNodeBridge::eOptFlags(eFlags));
}


int VistaSG::GetNumberOfMaterials(void)
{
	return m_pGraphicsBridge->GetNumberOfMaterials();
}

int VistaSG::AddMaterial(const VistaMaterial &Material)
{
	return m_pGraphicsBridge->AddMaterial(Material);
}

bool VistaSG::GetMaterialByIndex(int iIndex, VistaMaterial &oIn) const
{
	return m_pGraphicsBridge->GetMaterialByIndex(iIndex, oIn);
}

bool VistaSG::GetMaterialByName(const std::string &sName, VistaMaterial &oIn) const
{
	return m_pGraphicsBridge->GetMaterialByName(sName, oIn);
}

VistaTextNode *VistaSG::NewTextNode(VistaGroupNode *pParent, const std::string &sFontDesc, IVista3DText *pText)
{
	if(!pText)
	{
...
IVistaNode* VistaSG::CloneSubtree( IVistaNode* pNode, 
									VistaGroupNode* pNewParent )
{
	...
	switch( pNode->GetType() )
	{
		case VISTA_GROUPNODE:
		{
			...
			{
				IVistaNode *pChild = CloneSubtree( pOrigNode->GetChild(n), pNewNode );
			}
		...
		case VISTA_SWITCHNODE:
		{
			...
			for( unsigned int n=0; n < pOrigNode->GetNumChildren(); ++n)
			{
				...
				IVistaNode *pClone = CloneSubtree( pNode, pNewNode );
			}
		...
		case VISTA_LEAFNODE:
		{
			vkernerr << "[VistaSG::CloneSubtree]: Warning - found node of type LEAFNODE!"
		...
		case VISTA_SPOTLIGHTNODE:
		{
			...
			switch( pOrigNode->GetLightType() )
			{
				...
				default:
				{
					vkernerr << "[VistaSG::CloneSubtree] Error: Found light with unknown type!" << std::endl
								<< " Cannot handle this node - omitting." << std::endl;
					return NULL;
					break;
				}
		...
		default:
		{
			vkernerr << "[VistaSG::CloneSubtree] Error: Found unknown scenegraph node!" << std::endl;								
			break;
		}
	}
...
/*                                                                            */
/*============================================================================*/
ostream & operator<< ( ostream & out, const VistaSG & device )
{
	device.Debug ( out );
	return out;
}
