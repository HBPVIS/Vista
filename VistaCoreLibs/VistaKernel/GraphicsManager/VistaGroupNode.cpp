...
VistaGroupNode::VistaGroupNode(VistaGroupNode*			pParent,
...
{
	return (std::find( m_vecChildren.begin(),
			           m_vecChildren.end(),
			           pNode ) != m_vecChildren.end());
}
// ============================================================================
// ============================================================================
...
bool VistaGroupNode::InsertChild(IVistaNode* pChild, unsigned int nIndex)
{
		...
		(CanAddChild(pChild)) && (nIndex<=m_vecChildren.size()))
	{
		if(m_pBridge->InsertChild(pVistaChild->GetData(), nIndex, m_pData))
		{
			...
			else m_vecChildren.push_back(pChild);
...
IVistaNode* VistaGroupNode::DisconnectChild(unsigned int nChildindex)
{
	...
	if(IsValidIndex(nChildindex))
	{
		if(m_pBridge->DisconnectChild(nChildindex, m_pData))
		{
			...
			static_cast<VistaNode*>(m_vecChildren[nChildindex])->m_nLocalTransScore += GetTransformScore() + 1;
			...
			for (j=0; j<m_nNumChildren; j++)
				if(j<nChildindex)
					...
					if(j==nChildindex)
						//remember removed child in <pChild> to make sure not to have any dangling pointers
						pChild = m_vecChildren[j];
					else
						if(j>nChildindex)
