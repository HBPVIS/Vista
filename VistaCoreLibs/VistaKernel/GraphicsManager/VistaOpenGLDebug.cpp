...
void VistaOpenGLDebug::DebugProjectionMatrix ()
{
	...
	if (curProjMatrix[11] != 0.0)
	{
		// check other entries on zero
		if ( (curProjMatrix[1] != 0.0) ||
			 (curProjMatrix[2] != 0.0) ||
			 (curProjMatrix[3] != 0.0) ||
			 (curProjMatrix[4] != 0.0) ||
			 (curProjMatrix[6] != 0.0) ||
			 (curProjMatrix[7] != 0.0) ||
			 (curProjMatrix[12] != 0.0) ||
			 (curProjMatrix[13] != 0.0) ||
			 (curProjMatrix[15] != 0.0) )
		{
		...
		else
		{
			...
			if (bError == false)
			{
				double nFar    = curProjMatrix[14] / (curProjMatrix[10] + 1.0);
				double nNear   = curProjMatrix[14] / (curProjMatrix[10] - 1.0);
				double nRight  = nNear * (curProjMatrix[8] + 1.0) / curProjMatrix[0];
				double nLeft   = nNear * (curProjMatrix[8] - 1.0) / curProjMatrix[0];
				double nTop    = nNear * (curProjMatrix[9] + 1.0) / curProjMatrix[5];
				double nBottom = nNear * (curProjMatrix[9] - 1.0) / curProjMatrix[5];
...
void VistaOpenGLDebug::DebugModelMatrix ()
{
	...
	}
	{
		...
		double angR12 = acos (scalprodR12 / (lenR1 * lenR2));
		double angR13 = acos (scalprodR13 / (lenR1 * lenR3));
		double angR23 = acos (scalprodR23 / (lenR2 * lenR3));
	...
	if ( (curModelMatrix[3]  == 0.0) ||
		 (curModelMatrix[7]  == 0.0) ||
		 (curModelMatrix[11] == 0.0) ||
		 (curModelMatrix[15] == 1.0) )
	{
		...
		{
			rotX = asin (-curModelMatrix[9]/cos(rotY));
			rotZ = asin (-curModelMatrix[4]/cos(rotY));

			// test x rot result
			double m9  = -sin(rotX)*cos(rotY);
			double m10 =  cos(rotX)*cos(rotY);
			// test z rot result
			double m0 =  cos(rotY)*cos(rotZ);
			double m4 = -cos(rotY)*sin(rotZ);
			   ...
			   )
			{
				...
				rotZ = asin (-curModelMatrix[4]/cos(rotY));
				// test x rot result
				m9  = -sin(rotX)*cos(rotY);
				m10 =  cos(rotX)*cos(rotY);
				// test z rot result
				m0 =  cos(rotY)*cos(rotZ);
				m4 = -cos(rotY)*sin(rotZ);
		...
		{
			rotX = - asin (curModelMatrix[9]);
			rotZ = - asin (curModelMatrix[4]);
			// now check here, too
			// ...
		...
		DebugMsg (")\n");
		{
			...
			float mat[16];
			mat[0]  = (float)( cos(rotY)*cos(rotZ) );
			mat[1]  = (float)( sin(rotX)*sin(rotY)*cos(rotZ) + cos(rotX)*sin(rotZ) );
			mat[2]  = (float)( -cos(rotX)*sin(rotY)*cos(rotZ) + sin(rotX)*sin(rotZ) );
			mat[3]  = 0.0f;
			mat[4]  = (float)( -cos(rotY)*sin(rotZ) );
			mat[5]  = (float)( -sin(rotX)*sin(rotY)*sin(rotZ) + cos(rotX)*cos(rotZ) );
			mat[6]  = (float)( cos(rotX)*sin(rotY)*sin(rotZ) + sin(rotX)*cos(rotZ) );
			mat[7]  = 0.0f;
			mat[8]  = (float)( sin(rotY) );
			mat[9]  = (float)( -sin(rotX)*cos(rotY) );
			mat[10] = (float)( cos(rotX)*cos(rotY) );
			mat[11]  = 0.0f;
