...
public:
	...
	/** Generate a new OpenGL node
	  * @param VistaGroupNode* pParent : Pointer to node's parent (NULL if node is intended to be orphan)
	  * @param IVistaOpenGLDraw* pDI : Pointer to draw interface containing the OpenGL calls that will be executed upon rendering
	...
	*/
	...
	/** Generate a new Extension node
	  * @param VistaGroupNode* pParent : Pointer to node's parent (NULL if node is intended to be orphan)
	  * @param IVistaExplicitCallbackInterface* pDI : Pointer to callback interface that will be executed upon rendering
	  * @return VistaExtensionNode* : Pointer to newly created node
	*/
	VistaExtensionNode *NewExtensionNode(VistaGroupNode *pParent, IVistaExplicitCallbackInterface *pExt);
	...
	*/
	...
	VistaTextNode *NewTextNode(VistaGroupNode *pParent, const std::string &sFontDesc = "Helvetica", IVista3DText *pTextI = NULL);
	...
	*/
	...
	VistaGeometry*		NewIndexedGeometry(
	...
	*/
	...
	/** Returns the root node of the SG if new GM is used. Otherwise use VistaOldGraphicsManager::GetSceneGraphRoot()
	  * @return VistaGroupNode* : Pointer to scene graph's root node
	*/
	/**
	 * @todo maybe make this a group node also?
	 * may make sense to leave THIS one a transformnode for world-in-hand-type interaction...
	 */
	...
	*/
	virtual IVistaNode*				GetNode (const std::string &strNodeName,
		                                     IVistaNode *pSubRoot = NULL) const;
	...
	*/
	virtual VistaGroupNode*	LoadSubTree(const std::string &strFileName,
	...
	*/
	virtual VistaGeomNode*		LoadGeomNode(const std::string &strFileName,
							eOptFlags eOpts = OPT_NONE,
							 float fScale = 1.0f);
	...
	*/
	virtual IVistaNode * LoadNode( const std::string &sFileName,
					eOptFlags eOpts = OPT_NONE,
									   float fScale = 1.0f );

	bool ApplyOptimizationToNode( IVistaNode *pNode,
							   eOptFlags eFlags );
	//*****
...
protected:
	...
	//construct geometry from given geom data. protected, because user shouldn't see the 'data concept'
...
/*============================================================================*/
VISTAKERNELAPI std::ostream & operator<< ( std::ostream & out, const VistaSG & device );

inline IVistaNodeBridge* VistaSG::GetNodeBridge() const
{
	return m_pNodeBridge;
}
inline IVistaGraphicsBridge* VistaSG::GetGraphicsBridge() const
{
	return m_pGraphicsBridge;
}
inline VistaGeometry* VistaSG::NewGeometry(IVistaGeometryData* pData)
{
