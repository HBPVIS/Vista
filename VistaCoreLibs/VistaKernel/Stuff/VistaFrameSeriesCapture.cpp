...
void VistaFrameSeriesCapture::HandleEvent( VistaEvent* pEvent )
{
	switch( m_eCaptureMode )
	{
		case CM_EVERY_FRAME:
		{
			MakeScreenshot( pEvent->GetTime(), false );
			break;
		}
		case CM_EVERY_NTH_FRAME:
		{
			int nDelta = m_pSystem->GetFrameLoop()->GetFrameCount() - m_nLastCapture;
		...
		case CM_FIXED_FRAMERATE:
		{
			...
			{
				// first frame initializes our first time, only one render
				MakeScreenshot( pEvent->GetTime(), false );
				break;
			}

			VistaType::microtime nDelta = pEvent->GetTime() - m_nLastCaptureTime;
			while( nDelta > 2 * m_nPeriod )
			{
				MakeScreenshot( m_nLastCaptureTime + m_nPeriod, true );
				nDelta = pEvent->GetTime() - m_nLastCaptureTime;
			}
			if( nDelta > m_nPeriod )
				MakeScreenshot( m_nLastCaptureTime + m_nPeriod, false );
			break;
		}
		default:
			break;
...
bool VistaFrameSeriesCapture::StringReplace( std::string& sString, const std::string& sFind, const std::string& sReplace )
{
...
void VistaFrameSeriesCapture::ReplaceMilliSeconds( std::string& sName, const VistaType::microtime nTime )
{
	...
	m_bNeedsMilliReplace = StringReplace( sName, "%M%", sMilliString );
}
...
void VistaFrameSeriesCapture::ReplaceFrameCount( std::string& sName )
{
	...
	m_bNeedsFrameReplace = StringReplace( sName, "%F%", sCountString );
}
...
void VistaFrameSeriesCapture::ReplaceScreenshotCount( std::string& sName )
{
	...
	m_bNeedsScreenshotReplace = StringReplace( sName, "%S%", sCountString );
}
...
void VistaFrameSeriesCapture::MakeScreenshot( const VistaType::microtime nTime, bool bImmediate )
{
	...
	m_pSystem->GetDisplayManager()->MakeScreenshot( m_pWindow, sFilename, !bImmediate );

	m_nLastCapture = m_pSystem->GetFrameLoop()->GetFrameCount();
