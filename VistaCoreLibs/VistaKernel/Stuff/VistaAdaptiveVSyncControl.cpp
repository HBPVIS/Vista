...
void VistaAdaptiveVSyncControl::HandleEvent( VistaEvent* pEvent )
{
	...
	if( bVSyncStatus && nDeltaT > ( 1.0f / m_nSwitchAtFramerate ) * ( 1 + m_nFramerateTolerance ) )
	{
	...
	else if( !bVSyncStatus && nDeltaT < ( 1.0f / m_nSwitchAtFramerate ) * ( 1 - m_nFramerateTolerance ) )
	{
		++m_nFramesExceedingLimit;
