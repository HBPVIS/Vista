...
static ACObject *ac3d_load_object(FILE *f, ACObject *parent, ACMaterial *pPalette, int &iNumPalette, char *buff, int &line)
{
	char t[20];
	ACObject *ob = NULL;
...
static std::string ConstructMaterialName(const std::string &sFileName, 
										 const std::string &sMatNam, int iIndex)
{
							 ...
							 VistaGroupNode *pParent, ACObject *pObj, std::vector<ACMaterial> &mat)
{
	...
	{
		if(pObj->num_surf == 1 
			&& ((pObj->surfaces[0].flags & AC3D_SURFACE_TYPE_LINE)
	...
	else if(pObj->type == AC3D_OBJECT_GROUP || pObj->type == AC3D_OBJECT_WORLD)
	{
		...
		VistaTransformMatrix m(pObj->matrix[0], pObj->matrix[1], pObj->matrix[2], pObj->loc.x,
								  pObj->matrix[3], pObj->matrix[4], pObj->matrix[5], pObj->loc.y,
								  pObj->matrix[6], pObj->matrix[7], pObj->matrix[8], pObj->loc.z,
								  0,0,0, 1);
...
//	normals.resize(obj->num_vert);
	...
	{
		coords[i] = VistaVector3D(obj->vertices[i].x,
								   obj->vertices[i].y, 
								   obj->vertices[i].z);
...
//			obj->vertices[i].normal.y, obj->vertices[i].normal.z);
	...
	for(int j=0; j < obj->num_surf; ++j)
	{
		...
		for(int k=0; k < obj->surfaces[j].num_vertref; ++k)
		{
			...
			textureCoords.push_back(VistaVector3D(obj->surfaces[j].uvs[k].u
												   *float(obj->texture_repeat_x)
												   +obj->texture_offset_x,
													obj->surfaces[j].uvs[k].v
												   *float(obj->texture_repeat_y)
												   +obj->texture_offset_y, 0));

			vertices.push_back(vert);
		}
	}

	VistaGeometry *geom = pSG->NewIndexedGeometry(vertices, coords, textureCoords, normals, colors, format);
	if(geom)
	{
		VistaMaterial oMat;
		...
		if(obj->texture)
		{
			...
			{
				std::string	sPrefix = sFileName.substr(0, 1+sFileName.rfind(VistaFileSystemDirectory::GetOSSpecificSeparator()));
		...
		att.culling = iCullFlag ? VistaRenderingAttributes::CULL_NONE
			: VistaRenderingAttributes::CULL_BACK;	
		...
		VistaTransformMatrix mat(obj->matrix[0], obj->matrix[1], obj->matrix[2], obj->loc.x,
								  obj->matrix[3], obj->matrix[4], obj->matrix[5], obj->loc.y,
								  obj->matrix[6], obj->matrix[7], obj->matrix[8], obj->loc.z,
								  0,0,0, 1);
								   ...
								   ACObject *obj, std::vector<ACMaterial> &mat)
{
	...
	pContainer->SetIsClosedLine((obj->surfaces[0].flags & AC3D_SURFACE_TYPE_CLOSEDLINE) ? true:false);
									   ...
									   const std::string &sTexturePathPrefix)
{
	...
	for(int i=0; i < iNumMaterials; ++i)
	{
		...
		VistaMaterial oMat(ambient, diffuse, specular, emmissive, (*mat).shininess, 1-(*mat).transparency, sName);
