...
bool   VistaDfnViewerSourceNode::DoEvalNode()
{
	...
	VistaDisplaySystem::VistaDisplaySystemProperties *pProps = m_pSystem->GetDisplaySystemProperties();
	...
	if(nChg & DispObserver::MSG_POS_CHANGE)
	{
		...
		{
			// world viewer		}
			VistaVector3D v3Trans(m_pSystem->GetReferenceFrame()->TransformPositionToFrame(m_pPosition->GetValueConstRef()));
	...
	if(nChg & DispObserver::MSG_ORI_CHANGE)
	{
		...
		{
			VistaQuaternion q(m_pSystem->GetReferenceFrame()->TransformOrientationToFrame(m_pOrientation->GetValueConstRef()));
