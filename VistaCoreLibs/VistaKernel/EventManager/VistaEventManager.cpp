...
{
	bool operator()(const VistaEventManager::HANDLER &left,
		const VistaEventManager::HANDLER &right) const
	{
...
/*                                                                            */
/*============================================================================*/
bool VistaEventManager::UnregisterObserver(VistaEventObserver *pObserver,
											int iEventType /* = VistaEvent::VET_ALL */)
{
	if (!pObserver)
		return  false;
...
bool VistaEventManager::RemEventHandler(VistaEventHandler *pHan,
										 EVENTTYPE nEventType,
										 EVENTID nEventId)
{
	...
	if(nEventType > 0 &&
	   nEventType > EVENTTYPE(m_veHandlerMapping.size()))
		return false;
...
VistaEventManager::EVENTID  VistaEventManager::RegisterEventId(int nEventType,
										 const std::string &sDebugString)
{
...
bool VistaEventManager::CleanupHandlerMapping()
{
	...
	for(std::vector< HANP >::size_type nIdx = 0;
		nIdx < m_veHandlerMapping.size(); ++nIdx)
	{
		...
		for(std::vector<HQUEUE>::size_type nIdIdx = 0;
			nIdIdx < h.size(); ++nIdIdx)
		{
...
VistaEventManager::HANQUEUE *VistaEventManager::GetHandlerList(EVENTTYPE nEventType,
