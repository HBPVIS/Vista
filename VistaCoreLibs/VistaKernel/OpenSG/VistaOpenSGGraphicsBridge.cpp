...
bool VistaOpenSGGeometryData::SetAttrChunk(osg::PolygonChunkPtr ptr)
{
	...
	if(m_ptrAttrChunk != osg::NullFC)
	{
		...
		endEditCP(mt);
		// what about the ref-counting
		//subRefCP(m_pAttrChunk); // killed?
...
bool VistaOpenSGGeometryData::GetRenderingAttributes( VistaRenderingAttributes & renderingattributes ) const
{
	renderingattributes = m_pRenderingAttributes;
	return true;
}

bool VistaOpenSGGeometryData::SetRenderingAttributes( const VistaRenderingAttributes & renderingattributes )
{
...
VistaOpenSGGraphicsBridge::VistaOpenSGGraphicsBridge(
	osg::RenderAction * pRenderAction, 
...
bool VistaOpenSGGraphicsBridge::CalcVertexNormals(IVistaGeometryData* pData, const float &fCreaseAngle)
{
	OSG::calcVertexNormals(dynamic_cast<VistaOpenSGGeometryData*>(pData)->GetGeometry(), fCreaseAngle);
...
//      VistaGroupNode * TheRootOfAllEvil = GetVistaSystem()->GetGraphicsManager()->GetSceneGraph()->GetRoot();
//      if(TheRootOfAllEvil != NULL)
//      {
//              VistaOpenSGNodeData * pOpenSGNode = static_cast<VistaOpenSGNodeData*>(TheRootOfAllEvil->GetData());
//              NodePtr contentNode = pOpenSGNode->GetContentNode() ;
...
bool VistaOpenSGGraphicsBridge::SetCoordinate(const int idx, const VistaVector3D& coord, IVistaGeometryData* pData)
{
	...
	pOpenSGData->GetGeometry()->getPositions()->setValue( osg::Pnt3f(coord[0],coord[1],coord[2]), idx );
	endEditCP(pOpenSGData->GetGeometry()->getPositions());

	return true;
}

bool VistaOpenSGGraphicsBridge::SetCoordinate(const int idx, const float coord[], IVistaGeometryData* pData)
{
	VistaOpenSGGeometryData* pOpenSGData = static_cast<VistaOpenSGGeometryData*>(pData);
...
bool VistaOpenSGGraphicsBridge::SetCoordinates(const int idx, const vector<VistaVector3D>& coords, IVistaGeometryData* pData)
{
...
bool VistaOpenSGGraphicsBridge::SetTextureCoord2D(const int idx, const VistaVector3D& coord, IVistaGeometryData* pData)
{
	VistaOpenSGGeometryData* pOpenSGData = static_cast<VistaOpenSGGeometryData*>(pData);
	...
	pOpenSGData->GetGeometry()->getTexCoords()->setValue( osg::Vec2f(coord[0], coord[1]), (unsigned int)idx );
	endEditCP(pOpenSGData->GetGeometry()->getTexCoords());
	return true;
}

bool VistaOpenSGGraphicsBridge::SetTextureCoord2D(const int idx, const float coord[], IVistaGeometryData* pData)
{
	VistaOpenSGGeometryData* pOpenSGData = static_cast<VistaOpenSGGeometryData*>(pData);
	if( idx < 0 || (unsigned int)idx >= pOpenSGData->GetGeometry()->getTexCoords()->size() )
		return false;
...
bool VistaOpenSGGraphicsBridge::SetNormal(const int idx, const VistaVector3D& normal, IVistaGeometryData* pData)
{
	return SetNormal(idx, &normal[0], pData);
}

bool VistaOpenSGGraphicsBridge::SetNormal(const int idx, const float normal[3], IVistaGeometryData* pData)
{
	VistaOpenSGGeometryData* pOpenSGData = static_cast<VistaOpenSGGeometryData*>(pData);
...
bool VistaOpenSGGraphicsBridge::SetColor(const int idx, const VistaColor& color, IVistaGeometryData* pData)
{
	...
	if( colors == osg::NullFC )
	{
		...
		if( pMt != osg::NullFC )
		{
			...
			if(mt != osg::NullFC)
			{
				osg::MaterialChunkPtr matchunk = osg::MaterialChunkPtr::dcast(mt->find(osg::MaterialChunk::getClassType()));
...
bool VistaOpenSGGraphicsBridge::SetCoordIndex(const int idx, const int value, IVistaGeometryData* pData)
{
	VistaOpenSGGeometryData* pOpenSGData = static_cast<VistaOpenSGGeometryData*>(pData);
...
bool VistaOpenSGGraphicsBridge::SetTextureCoordIndex(const int idx, const int value, IVistaGeometryData* pData)
{
	VistaOpenSGGeometryData* pOpenSGData = static_cast<VistaOpenSGGeometryData*>(pData);
...
bool VistaOpenSGGraphicsBridge::SetNormalIndex(const int idx, const int value, IVistaGeometryData* pData)
{
	VistaOpenSGGeometryData* pOpenSGData = static_cast<VistaOpenSGGeometryData*>(pData);
...
bool VistaOpenSGGraphicsBridge::SetColorIndex(const int idx, const int value, IVistaGeometryData* pData)
{
	VistaOpenSGGeometryData* pOpenSGData = static_cast<VistaOpenSGGeometryData*>(pData);
...
bool VistaOpenSGGraphicsBridge::SetColor(const VistaColor & color, IVistaGeometryData* pData)
{
	...
	else
	{
		...
		if(matChunk == osg::NullFC)
		{
			...
			matChunk->setDiffuse(colAmbDiff);
			matChunk->setAmbient(colAmbDiff);
			matChunk->setSpecular(osg::Color4f(1,1,1,opacity));
			matChunk->setEmission(osg::Color4f(0,0,0,opacity));
			matChunk->setShininess(20.0f);
			matChunk->setLit(GL_TRUE);
			endEditCP(matChunk);

			beginEditCP(stm);
			stm->addChunk(matChunk); // add to material
			endEditCP(stm);
		}
		else
		{
...
bool VistaOpenSGGraphicsBridge::SetTexture(osg::ImagePtr image, IVistaGeometryData* pData)
{
	...
	osg::TextureChunkPtr texchunk  = osg::TextureChunkPtr::dcast(mt->find(osg::TextureChunk::getClassType(),0));
	if(texchunk == osg::NullFC)
	{
		texchunk = osg::TextureChunk::create();
		beginEditCP(texchunk,
			 osg::TextureChunk::MinFilterFieldMask |
...
bool VistaOpenSGGraphicsBridge::SetTexture(const int id,
		                                    const int width,
		                                    const int height,
		                                    const int bpp,
		                                    ...
		                                    IVistaGeometryData* pData)
{
	...
	beginEditCP(image);
	image->set( (bHasAlpha ? osg::Image::OSG_RGBA_PF : osg::Image::OSG_RGB_PF),
			    width,
			    height,
			    1, 1, 1, 0,
			    pBuffer,
...
bool VistaOpenSGGraphicsBridge::ScaleGeometry(const float sx, const float sy, const float sz, IVistaGeometryData* pData)
{
	VistaOpenSGGeometryData* pOpenSGData = static_cast<VistaOpenSGGeometryData*>(pData);
	osg::Vec3f cog(0,0,0), pos;
	unsigned int i, n(pOpenSGData->GetGeometry()->getPositions()->size());
										...
										const IVistaGeometryData* pData) const
{
	...
	for( i = 1; i < n; ++i )
	{
		...
		if( coordinate[0] < pMin[0] ) pMin[0] = coordinate[0];
		if( coordinate[1] < pMin[1] ) pMin[1] = coordinate[1];
		if( coordinate[2] < pMin[2] ) pMin[2] = coordinate[2];
		if( coordinate[0] > pMax[0] ) pMax[0] = coordinate[0];
		if( coordinate[1] > pMax[1] ) pMax[1] = coordinate[1];
		if( coordinate[2] > pMax[2] ) pMax[2] = coordinate[2];
	}
	return true;
}
							...
							const IVistaGeometryData* pData) const
{
	...
	case VistaGeometry::VISTA_FACE_TYPE_UNKNOWN: // mixed face type
	{
		...
		for(osg::UInt32 i=1; i<verticesPerFace; ++i)
		{
			...
			if(pos[0] < pmin[0]) pmin[0] = pos[0];
			if(pos[1] < pmin[1]) pmin[1] = pos[1];
			if(pos[2] < pmin[2]) pmin[2] = pos[2];
			if(pos[0] > pmax[0]) pmax[0] = pos[0];
			if(pos[1] > pmax[1]) pmax[1] = pos[1];
			if(pos[2] > pmax[2]) pmax[2] = pos[2];
		}
		return true;
	}
	default:
	...
	osg::Pnt3f pos(geo->getPositions()->getValue
		(indices->getValue(verticesPerFace*idx * mappingSize +
											...
											const IVistaGeometryData* pData) const
{
	...
	a = VistaVector3D( &(geo->getPositions()->getValue(indices->getValue((3*idx+0) * mappingSize)))[0] );
	b = VistaVector3D( &(geo->getPositions()->getValue(indices->getValue((3*idx+1) * mappingSize)))[0] );
	c = VistaVector3D( &(geo->getPositions()->getValue(indices->getValue((3*idx+2) * mappingSize)))[0] );
	return true;
}
...
bool VistaOpenSGGraphicsBridge::AddFace(const int vertexId0, const int vertexId1, const int vertexId2, int& faceId, IVistaGeometryData* pData)
{
	VistaOpenSGGeometryData * pOpenSGData = static_cast<VistaOpenSGGeometryData*>(pData);
	osg::GeometryPtr geo = pOpenSGData->GetGeometry();
...
bool VistaOpenSGGraphicsBridge::DeleteFace(const int faceId, bool deleteVertices, IVistaGeometryData* pData)
{
	return false;
}
...
bool VistaOpenSGGraphicsBridge::SetRenderingAttributes(const VistaRenderingAttributes& attr, IVistaGeometryData* pData)
{
...
bool VistaOpenSGGraphicsBridge::CreateIndexedGeometry
...
bool VistaOpenSGGraphicsBridge::DeleteGeometryData(IVistaGeometryData* pData)
{
	VistaOpenSGGeometryData * pOpenSGData = static_cast<VistaOpenSGGeometryData*>(pData);
	delete pOpenSGData;
	return true;
...
bool VistaOpenSGGraphicsBridge::SetMaterial(const VistaMaterial& oMaterial, IVistaGeometryData* pData)
{
	...
	osg::ChunkMaterialRefPtr stm(osg::ChunkMaterialPtr::dcast(pOpenSGData->GetGeometry()->getMaterial()));
	if( stm == osg::NullFC )
		return false;
...
bool VistaOpenSGGraphicsBridge::SetMaterialIndex(const int& materialIndex, IVistaGeometryData* pData)
{
	...
	//{
	//	osg::MaterialChunkPtr matchunk = osg::MaterialChunkPtr::dcast(stm->find(MaterialChunk::getClassType()));
	...
	//	osg::BlendChunkPtr blendchunk = osg::BlendChunkPtr::dcast(stm->find(osg::BlendChunk::getClassType()));
	//	osg::Real32 opacity = 1.0f;
	//	if(blendchunk == osg::NullFC)
	//	{
