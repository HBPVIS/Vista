...
/* IMPLEMENTATION                                                             */
/*============================================================================*/
VistaOSGThreadImp::VistaOSGThreadImp(const std::string &sName, 
									   VistaThread *pThread,
									   int nAspect)
{
 ...
 bool  VistaOSGThreadImp::Run( )
{
	if(m_pOSGThread == NULL)
	{
		...
		{
			m_pOSGThread = dynamic_cast<OSG::Thread*>
					   (OSG::ThreadManager::the()->getThread(GetThreadName().c_str()));
		}
		else
			m_pOSGThread = dynamic_cast<OSG::Thread*>
					   (OSG::ThreadManager::the()->getThread(NULL));
