...
{		
	...
	try
	{
		...
		pConn->SetByteorderSwapFlag( false );
	...
	try
	{
		...
		pSyncConn->SetByteorderSwapFlag( false );
...
void VistaNewClusterMaster::PrintClusterSetupInfo()
{
	...
	vstr::outi() << "SwapSync timeout : " << vstr::formattime( m_nSwapSyncTimeout, 3 );
}
...
void VistaNewClusterMaster::Debug( std::ostream& oStream ) const
{
	...
	oStream << "    SwapSync timeout : " << vstr::formattime( m_nSwapSyncTimeout, 3 );
