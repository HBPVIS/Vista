...
public:
	...
	Slave( const std::string &sSlaveName,
			...
			bool bDoSync )
	: m_sIP( pCon->GetPeerName()), m_nPort(pCon->GetPeerPort() )
	, m_pCon( pCon )
	, m_sName( sSlaveName )
...
public:
	...
	, m_nCounter(0)
	{
		m_pSerializer->SetByteorderSwapFlag( bDoSwap );
	}
...
namespace
{
	...
	VistaConnectionIP* CreateConnectionToSlave( Slave* pSlave )
	{
		try
		{
			...
			pConnection->SetByteorderSwapFlag( false );
			return pConnection;
		}
...
public:
	...
	{
		SetupBuffers( m_vecSystemBuffers,
	...
	void BroadcastEvent( EventBuffer *pBuffer, bool bEndOfFrame )
	{
			 ...
			 itSlave != m_vecSendList.end(); ++itSlave )
		{
			...
			try
			{
				...
				if(nL == 0)
				{
					vstr::errp() << "[ViClMaster]: error during WaitForSendFinish() ["
							  << (*itSlave)->m_sName << "]\n"
			...
			{
				vstr::errp() << "[ViClMaster]: error during send to slave ["
						<< (*itSlave)->m_sName 
	...
	void BroadcastTime( VistaType::systemtime dTime )
	{
			 ...
			 itSlave != m_vecSendList.end(); ++itSlave )
		{
			...
			try
			{
				...
				// note that the write starts at the (*pBuffer).GetBuffer()
				// and writes the complete buffer, including the header
				int nRet = pCon->WriteRawBuffer( m_oSerializer.GetBuffer(),
												m_oSerializer.GetBufferSize() );
			...
			{
				vstr::errp() << "[ViClMaster]: error during send to client ["
	...
	bool EstablishConnection( Slave* pSlave, double dFrameClock )
	{
		...
		try
		{
			pEventChannel = new VistaConnectionIP( 
												VistaConnectionIP::CT_TCP,
												pSlave->m_sIP, pSlave->m_nPort );
			
			//pEventChannel->SetByteorderSwapFlag( pSlave->m_bSwap );
			// since the slaves know if they have to swap, we never do so
			pEventChannel->SetByteorderSwapFlag( false );
			if( pEventChannel->GetIsOpen() )
			{
				...
				pAckChannel = new VistaConnectionIP( VistaConnectionIP::CT_TCP,
													pSlave->m_sIP, nAckPort );
				if( pAckChannel->GetIsOpen() )
				{
					pAckChannel->SetIsBlocking(true);
					pAckChannel->SetIsBuffering(false);
					pAckChannel->SetByteorderSwapFlag( false );
				...
				{
					// connection failed.
					vstr::warnp() << "Failed to init ack channel at IP [" << pSlave->m_sIP 
							<< " - Port [" << nAckPort << "]" << std::endl;
			 ...
			 itSlave != m_vecSendList.end(); ++itSlave )
		{
			...
			if( bRefreshSerializer )
			{
				...
				for( std::vector<Slave*>::iterator itSlave = m_vecSlaves.begin();
						itSlave != m_vecSlaves.end(); ++itSlave )
				{
					nMessageSize += m_oSerializer.WriteInt32( (VistaType::sint32)(*itSlave)->m_sName.size() );
					nMessageSize += m_oSerializer.WriteString( (*itSlave)->m_sName );
					nMessageSize += m_oSerializer.WriteBool( ( (*itSlave)->m_pCon != NULL ) );
				}
				...
				memcpy( const_cast<VistaType::byte*>( m_oSerializer.GetBuffer() ), &nWriteSize, sizeof(VistaType::sint32) );
			}
	...
	{
		for( std::vector<Slave*>::const_iterator it = m_vecSlaves.begin();
			 it != m_vecSlaves.end(); ++it )
		{
	...
	{
		std::vector<Slave*>::iterator itSlave = std::find( m_vecSendList.begin(),
															m_vecSendList.end(),
															pSlave );
		if( itSlave != m_vecSendList.end() )
			m_vecSendList.erase( itSlave );

		itSlave = std::find( m_vecAckList.begin(),
								m_vecAckList.end(),
								pSlave );
...
/*============================================================================*/
VistaClusterMaster::VistaClusterMaster( VistaEventManager *pEventManager,
...
void VistaClusterMaster::SwapSync()
{
	    ...
	    itSlave != m_pEventObserver->m_vecAckList.end(); ++itSlave )
	{
		try
		{
			...
			{
				vstr::errp() << "[ViClMaster]: Slave ["
					<< (*itSlave)->m_sName << "] failed during ack" << std::endl;
				liRemoveList.push_back( *itSlave );
			}
			else if( nReadToken != (int)m_pEventObserver->m_nFrameIndex ) // see whether frame index matches
			{
				vstr::errp() << "[ViClMaster]: slave ["
				          << (*itSlave)->m_sName << "] failed to ack correct"
						  << " frame token" << std::endl;
				vstr::errp() << "[ViClMaster]: expected [" << m_pEventObserver->m_nFrameIndex << "] but was ["
					<< nReadToken << "], socket: [" << nRet << "]" << std::endl;
				liRemoveList.push_back( *itSlave );
			}
		}
		catch(VistaExceptionBase &x)
...
bool VistaClusterMaster::CreateConnections(
...
void VistaClusterMaster::Debug( std::ostream& oStream ) const
{
	...
	{
		oStream << "["   << (*cit)->m_sName << "]\n";
		oStream << "\t" << ( ( (*cit)->m_pCon == NULL ) ? ( "!DEAD!") : ( "ALIVE" ) )
			<< (*cit)->m_sIP
			<< " ; " << (*cit)->m_nPort
			<< " ; "
			...
			<< " ; "
			<< ((*cit)->m_bSwapSyncs ? "DOES SWAPSYNC" : "NO SWAPSYNC")
			<< std::endl;
	}
}

bool VistaClusterMaster::Init( const std::string& sClusterSection,
								const VistaPropertyList& oConfig )
{
	vstr::outi() << "[VistaClusterMaster]: Initializing" << std::endl;
	{
			 ...
			 itSlave != liSlaveList.end(); ++itSlave )
		{
			...
			{
				vstr::warnp() << "ClusterMaster::Init() -- Slave ["
					<< (*itSlave) << "] specified, but no section exists" << std::endl;
				continue;
			}
...
{
	vstr::outi() << "\n[ViClMaster]: Main Initialitation finished - Waiting for ACK from Slaves." << std::endl;
	vstr::IndentObject oIndent;
	    ...
	    cit != m_pEventObserver->m_vecSlaves.end(); ++cit )
	{
		...
		{
			vstr::outi() << std::endl;
			vstr::errp() << "Exception during wait for alive signal from slave ["
					<< (*cit)->m_sName << "]:\n" 
					<< x.GetPrintStatement() << std::endl;
			m_pEventObserver->DeactivateSlave( (*cit) );
			continue;
...
bool VistaClusterMaster::AddSlave( const std::string& sName,
