														...
														const std::string &sReason) const
{
	...
	vstr::warni() << vstr::singleindent << " [VistaIM] Giving up. This connection will NOT work." << std::endl;
}
	...
	VistaConnection *CreateConnection(const VistaPropertyList &oProps)
		{
			std::string strHost;
			int nPort = -1;
			if( oProps.GetValue( "ADDRESS", strHost ) == false
				||oProps.GetValue( "PORT", nPort ) == false
				|| nPort <= 0 )
			{
										...
										const VistaPropertyList& oConfig )
{
	...
	if( oDriverSection.GetValue( "WINDOWS", liWindowNames )  )
	{
				...
				itWindow != liWindowNames.end(); ++itWindow )
		{
			...
			if( pWindow == NULL )
			{
				vstr::warnp() << "[VistaWindowConfigurator]: Driver requests Window ["
					<< (*itWindow) << "] which does not exist" << std::endl;
				continue;
			}
	...
	std::list<VistaDriverAbstractWindowAspect::WindowHandle> liCurrentWindows = pAsp->GetWindowList();
	for(std::list<VistaDriverAbstractWindowAspect::WindowHandle>::const_iterator cit = liCurrentWindows.begin();
	...
	for( std::list<VistaWindow*>::const_iterator itWin = liWindows.begin();
			itWin != liWindows.end(); ++itWin )
	{
		int nWindowId = (*itWin)->GetWindowProperties()->GetWindowId();
#if defined(WIN32)
												...
												const VistaPropertyList& oConfig )
{
		...
		itSensor != liSensors.end(); ++itSensor)
	{
		...
		if( pSensorAsp ) // driver has sensor mapping, so use this to set up the sensors...
		{
			...
			{
				vstr::warnp() << "[SensorMappingConfiguator]: Driver requests sensor ["
							<< (*itSensor) << ", its provided type [" << sType 
							<< "is invalid" << std::endl;
				continue;
			}			
		...
		itConnName != liConnections.end(); ++itConnName)
	{
		if( oConfig.HasSubList( (*itConnName) ) == false )
		{
			vstr::warnp() << "[VistaConnectionConfigurator]: Requested connection ["
						<< (*itConnName) << "] has no matching section" << std::endl;
			continue;
		}
		...
		{
			vstr::warnp() << "[VistaConnectionConfigurator]: Requested connection ["
						<< (*itConnName) << "] has no TYPE specified" << std::endl;
			continue;
		}
		...
		{
			vstr::warnp() << "[VistaConnectionConfigurator]: Requested connection ["
						<< (*itConnName) << "] has no DRIVERROLE specified" << std::endl;
			continue;
		}
		...
		{
			vstr::warnp() << "[VistaConnectionConfigurator]: Requested connection ["
						<< (*itConnName) << "] could not be created" << std::endl;
			return false;
		}
		else
		{
										...
										const VistaPropertyList& oConfig )
{
	...
	if( oSection.GetValue( "HEMISPHERE", sHemisphere ) )
	{
		...
		{
			vstr::warnp() << "[ReferenceFrameConfigurator]: Hemisphere parameter ["
			...
			cit != liSensorSections.end(); ++cit )
	{
		if( oConfig.HasSubList( (*cit) ) == false )
		{
			vstr::warnp() << "[ReferenceFrameConfigurator]: "
					<< "SENSOR_ALIGNEMT request sensor from non-existing section ["
					<< (*cit) << "] is invalid!" << std::endl;
		}
			...
			&& oSubProps.GetValue( "TRANSFORM", matTransform ) )
		{
			...
			{
				vstr::warnp() << "[ReferenceFrameConfigurator]: SetSensorAlignment for SensorID ["
		...
		{
			vstr::warnp() << "[ReferenceFrameConfigurator]: Sensor Alignment Section ["
					<< (*cit) << "] must contain valid entries for SENSOR_ID and ALIGNMENT!"
					<< std::endl;
		}
	}
										...
										const VistaPropertyList& oConfig )
{
	...
	if( pParamAspect == NULL )
		return false;
#ifdef DEBUG
	for( VistaPropertyList::const_iterator itProp = oSection.begin();
			itProp != oSection.end(); ++itProp )
	{
		if( pParamAspect->GetParameterContainer()->SetProperty( (*itProp).second ) == false )
		{
			vstr::warnp() << "[DriverParameterConfigurator]: Could not set parameter ["
