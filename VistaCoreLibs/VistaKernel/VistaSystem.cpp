...
struct DriverInfo
{
	...
	DriverInfo(IVistaDeviceDriver *pDriver,
...
namespace
{
	...
	std::string getAbsolutePath(const std::string& path)
	{
...
#ifdef WIN32
		//cerr << "[getAbsolutePath] pwd: " << VistaFileSystemDirectory::GetCurrentWorkingDirectory() << std::endl;
		char buf[MAX_PATH];
		...
		{
			vstr::errp() << "[ViSystem] \"realpath\" syscall failed for "
				<< path << std::endl;
			vstr::errp() << vstr::singleindent << "temporary workaround: try to build a pseudo-path on our own..." << std::endl;
			char *pwd = getcwd(NULL, 0);
			std::string newPath(pwd);
			free(pwd);
			newPath.append("/");
			newPath.append(path);
			vstr::errp() << vstr::singleindent << "approximated (non-checked) path: " << newPath << std::endl; 
			return newPath;
		}
#endif
	}
...
VistaSystem* GetVistaSystem () {return S_pVistaSystem;}
...
, m_bAllowStreamColors( true )
{
	...
	for( int n = VistaExternalMsgEvent::VEID_INCOMING_MSG; n < VistaExternalMsgEvent::VEID_LAST; ++n )
	{
...
bool VistaSystem::DoInit( int argc, char **argv )
{
	...
	{
		vstr::errp() << "VistaSystem::Init() - "
			<< "Config file [" << m_sVistaConfigFile 
			<< "] does not contain system section ["
	...
	
	{
		...
		m_oGraphicsConfig.GetValueInSubList( "GRAPHICSSECTION", GetSystemSectionName(), sGraphicsSectionName );

		m_pGraphicsManager->SetupScene( sGraphicsSectionName, m_oGraphicsConfig );

		bool bLoadModel = m_oGraphicsConfig.GetValueInSubListOrDefault<bool>( "displayactive", GetSystemSectionName(), true );
...
bool VistaSystem::LoadIniFiles()
{
	...
	else
	{
		...
		{
			vstr::errp() << "Could not find Display Config File ["
				<< m_sDisplayConfigFile  << "] not found!" << std::endl;
			return false;
		}
	...
	else
	{
		...
		{
			vstr::errp() << "Could not find Graphics Config File ["
				<< m_sGraphicsConfigFile  << "] not found!" << std::endl;
			return false;
		}
	...
	else
	{
		...
		{
			vstr::errp() << "Could not find Interaction Config File ["
				<< m_sInteractionConfigFile  << "] not found!" << std::endl;
			return false;
		}
	...
	else
	{
		...
		{
			vstr::errp() << "Could not find Cluster Config File ["
				<< m_sClusterConfigFile  << "] not found!" << std::endl;
			return false;
		}
...
bool VistaSystem::SetIniSearchPaths(const std::list<std::string> &liSearchPaths)
{
	...
	for(std::list<std::string>::const_iterator it = liSearchPaths.begin();
...
bool VistaSystem::SetupDisplayManager()
{
	...
	if( m_pDisplayManager->CreateDisplaySystems( sDisplaySystemSectionName, m_oDisplayConfig ) == false )
	{
	...
	bool bShowCursor = m_oGraphicsConfig.GetValueInSubListOrDefault<bool>( 
										"ShowCursor", sGraphicsSectionName,
...
bool VistaSystem::SetupBasicInteraction()
{
	...
	// SETUP INTERACTIONCONTEXTS
	{
		...
		m_oInteractionConfig.GetValueInSubList( "INTERACTIONCONTEXTS",
												GetSystemSectionName(), 
												liInteractionContexts );
		vstr::outi() << "Found " << liInteractionContexts.size() << " entries in INTERACTIONCONTEXTS" << std::endl;
			...
			itDisplay != m_mapDisplays.end(); ++itDisplay )
	{
		...
		VistaUserPlatform* pPlatform = new VistaUserPlatform( GetGraphicsManager()->GetSceneGraph(), pDisplaySystem );
		m_mapUserPlatforms[pDisplaySystem] = pPlatform; // add to memory (for later disposal during deconstruction)

		// register this with the object registry
		GetDfnObjectRegistry()->SetObject( std::string("CAM:") + pDisplaySystem->GetNameForNameable(),
										   pDisplaySystem->GetProperties(),
										   pPlatform->GetPlatformTrans() );
	}
...
{	
	...
	m_pKeyboardSystemControl->BindAction( 'q',
	...
	m_pKeyboardSystemControl->BindAction('C',
								 new VistaToggleCursorCommand(GetDisplayManager()),
								 "Enable / Disable Cursor");
	m_pKeyboardSystemControl->BindAction( 'V',
								 new VistaToggleVSyncCommand( GetDisplayManager() ),
...
VistaInteractionContext* VistaSystem::SetupInteractionContext( const std::string& sContextName )
{
	...
	vstr::outi() << "[VistaSystem::SetupContext()]: Creating interaction context ["
			<< sContextName << "]" << std::endl;
	vstr::IndentObject oIndent;

	if( m_oInteractionConfig.HasSubList( sContextName ) == false )
	{
		vstr::warnp() << "Section for interaction Context ["
					<< sContextName << "] does not exist" << std::endl;
		return NULL;
	}
	...
	{
		vstr::warnp() << "No role given for context ["
				<< sContextName << "]!" << std::endl;
	}
	...
	{
		vstr::warnp() 
					<< "No graph file defined for context [" << sRole << "]" << std::endl;
		return NULL;
	}
...
VistaUserPlatform *VistaSystem::GetPlatformFor( VistaDisplaySystem *pDSys ) const
{
	...
	std::map<VistaDisplaySystem*, VistaUserPlatform*>::const_iterator cit = m_mapUserPlatforms.find(pDSys);
...
bool VistaSystem::Run()
{
	...
	{
		vstr::errp() 
				<< "[VistaSystem]: Trying to Run non-initialized VistaSystem!" << std::endl;
		return false;
	}
...
{
	//cout << "*********** VSE_EXTERNAL_MESSAGE ************" << std::endl;
	//cout << "*********** " << ((VistaSystemEvent*)pEvent)->GetId() << "************" << std::endl;
...
{	
	...
	m_oInteractionConfig.GetValueInSubList( "DRIVERPLUGINDIRS", GetSystemSectionName(),
												sDriverPlugins );
	...
	m_oInteractionConfig.GetValueInSubList( "DEVICEDRIVERS", GetSystemSectionName(),
												liDriverList );
		...
		itDriver != liDriverList.end(); ++itDriver )
	{
		...
		{
			vstr::errp() << "No type name given for the device driver [" << (*itDriver) << "]" << std::endl;
			continue;
		}
		...
		if( !pDriverCRM )
		{
			...
			{
				vstr::warnp() << "Could not load Plugin for Driver of Type ["
							<< sType << "]" << std::endl;
				continue;
			}
		}		
	...
	if( m_pClusterMode->GetIsLeader() )
	{
			...
			itDriver != liDriverList.end(); ++itDriver)
		{
			...
			{
				vstr::errp() << "Section for device driver [" 
						<< *itDriver << "] does not exist" << std::endl;
				continue;
			}
			const VistaPropertyList& oDriverSection = m_oInteractionConfig.GetSubListConstRef( (*itDriver) );
			...
			{
				// yes. no-go!
				vstr::warnp() << "Device Driver river of type ["
							   + *itDriver + "] with name ["
							   + sName + "] already registered -- SKIPPING!" << std::endl;
				continue;
			}
			...
			if( pDriver == NULL )
			{
				vstr::warnp() << "Could not create driver [" << *itDriver 
							<< "] from factory method!" << std::endl;
				continue;
			}

			vstr::outi() << "Adding driver of type [" + sType
						+ "] with name [" + sName + "]" << std::endl;
		...
		// sort
		std::list<VistaTopologyGraph<DriverInfo*>::Node*> liConfigOrder = oDependencyGraph.CalcTopology();

		// iterate over this order.
		for(std::list<VistaTopologyGraph<DriverInfo*>::Node*>::const_iterator itDriverNode = liConfigOrder.begin();
			itDriverNode != liConfigOrder.end(); ++itDriverNode)
		{
			if( m_oInteractionConfig.HasSubList( (*itDriverNode)->m_oElement->m_strSection ) == false )
			{
				vstr::warnp() << "Could not initialize driver ["
					 << (*itDriverNode)->m_oElement->m_strName << "] - section ["
					 << (*itDriverNode)->m_oElement->m_strSection << "] does not exist" << std::endl;
				continue;
			}
			vstr::outi() << "Creating and initializing driver ["
					 << (*itDriverNode)->m_oElement->m_strName << "] from section ["
					 << (*itDriverNode)->m_oElement->m_strSection << "]" << std::endl;

			const VistaPropertyList& oDriverSection = m_oInteractionConfig.GetSubListConstRef( 
														(*itDriverNode)->m_oElement->m_strSection );
					...
					itKey != oDriverSection.end(); ++itKey )
			{
				...
				if( liDependsOn.empty() )
				{
					...
					if( !liConfigDeps.HasNodeValue( (*itKey).first ) ) // this checks the case when a user has given
				...
				else
				{
						...
						cit != liDependsOn.end(); ++cit)
					{
						...
						if( oDriverSection.find( (*cit) ) != oDriverSection.end() )
							liConfigDeps.AddDependency( (*cit), (*itKey).first ); // yes, add only iff part of the configuration
						else
						{
							if(!liConfigDeps.HasNodeValue( (*itKey).first ) ) // not a node, yet...
								liConfigDeps.AddNode( (*itKey).first ); // add the node without dependency, as a floating node
						}
					}
				}
			}
				...
				keyit != liConfig.end(); ++keyit)
			{
				...
				if( pConf == NULL )
				{
						...
						&& (*keyit)->m_oElement != "TRANSCODERPLUGIN" )
					{
						vstr::warnp() 
								...
								<< (*keyit)->m_oElement << "] in section of driver ["
								<< (*itDriverNode)->m_oElement->m_strName << "]" << std::endl;
					}
					continue;
				}
				...
				if( pConf->Configure( (*itDriverNode)->m_oElement->m_pDriver, 
							oDriverSection, m_oInteractionConfig ) == false )
				{
					vstr::errp() << "Unable to configure element type ["
						<< (*keyit)->m_oElement << "]" << std::endl;
				}
			}
			...
			{
				vstr::errp() 
						<< "Could not successfully connect the driver,"
			...
			m_pInteractionManager->AddDeviceDriver( (*itDriverNode)->m_oElement->m_strName,
													  (*itDriverNode)->m_oElement->m_pDriver,
													  (*itDriverNode)->m_oElement->m_nPrio );
				...
				itSensor != liSensorNames.end(); ++itSensor )
			{
				...
				const VistaPropertyList& oSensorSection = m_oInteractionConfig.GetSubListConstRef( (*itSensor) );
	...
	{
		// in follower mode, we need a driver that is capable of receiving the incoming sensor data during
				...
				itDriver != liDriverList.end(); ++itDriver )
		{
			...
			IVistaDriverCreationMethod* pCreationMethod = m_pInteractionManager->GetDriverCreationMethod(sType);
			...
			unsigned int nPrio = oDriverSection.GetValueOrDefault<unsigned int>( "PRIORITY", 0 );
			mapCreatedDrivers[sName] = DriverInfo(pDriver, sType, sName, (*itDriver), nPrio, std::list<std::string>());
		}
			...
			itDriver != mapCreatedDrivers.end(); ++itDriver )
		{
			...
			const VistaPropertyList& oDriverSection = m_oInteractionConfig.GetSubListConstRef( itDriver->second.m_strSection );
			...
			IVistaDriverCreationMethod *pOrigin = m_pDriverMap->GetDriverCreationMethod( (*itDriver).second.m_strTypeName );
			if( pOrigin == NULL )
			{
			...
			else
			{
					...
					itSensor != liSensorNames.end(); ++itSensor )
				{
					...
					{
						vstr::warnp() << "Requested unknown sensor type ["
								<< sSensorType << "] for driver [" << (*itDriver).first << "]" 
								<< std::endl;
						continue;
					}
					IVistaMeasureTranscoderFactory *pTranscoderFac = pOrigin->GetTranscoderFactoryForSensor( nType );
					...
					else
					{
						...
						VistaDriverMeasureHistoryAspect *pHist
							= dynamic_cast<VistaDriverMeasureHistoryAspect*>(
									(*itDriver).second.m_pDriver->GetAspectById( VistaDriverMeasureHistoryAspect::GetAspectId() ) );
						if(pHist) // should never be NULL
						{
							...
							{
								vstr::warnp() << "DriverCreationMethod of driver ["
										<< (*itDriver).first << "] does not report an Update Rate for Sensor Type ["
										<< sSensorType << "]" << std::endl;
								continue;
							}
			...
			{
				m_pInteractionManager->AddDeviceDriver( (*itDriver).second.m_strName,
								(*itDriver).second.m_pDriver, (*itDriver).second.m_nPrio );
			...
			{
				vstr::errp() 
						<< "Could not sucessfully connect the driver,"
						<< " so it will not be enabled / registered." << std::endl;
...
bool VistaSystem::IndicateApplicationProgress(int iAppProgress, const std::string& sProgressMessage)
{
	...
	{
		return m_pMessagePort->WriteProgress(VistaKernelMsgPort::PRG_APP, iAppProgress, sProgressMessage);
	}
	return false;
}
...
{
	std::list<VistaKernelMsgPort*>::iterator it = std::find(m_liAppMsgPorts.begin(),
															 m_liAppMsgPorts.end(), pPort);
...
{
	oOut << " [ViSystem] ViSTA Version : " << VistaVersion::GetVersion() << "(" << VistaVersion::GetReleaseName() << ")";
...
std::string VistaSystem::FindFileInIniSearchPath(const std::string& filename, std::ostream *logger) const
{
#ifdef DEBUG
	if(!logger) logger = &vstr::out();
#endif
									...
									const std::string& sTranscoderName )
{
	...
	{
		vstr::warnp() << "Cannot find Driver Plugin file [" 
			<< sPluginFilename << "]" << std::endl;
		return false;
	}
	if( sActualTranscoderFile.empty() )
	{
		vstr::warnp() << "Cannot find Driver Plugin file [" 
			<< sTranscoderFilename << "]" << std::endl;
		return false;
	}
	...
	if( VddUtil::LoadTranscoderFromPlugin( sActualTranscoderFile, &oPlugin ) == false )
	{
		...
		vstr::warnp() << "[VistaSystem]: Loading of driver plugin ["
	...
	{
		vstr::warnp() << "[VistaSystem]: Loading of plugin ["
	...
	if( oPlugin.m_strDriverClassName.empty() )
	{
		vstr::warnp() << "[VistaSystem]: Loading of plugin for driver ["
			<< sDriverType << "] failed - plugin loaded, but has no name" << std::endl;
		VddUtil::DisposePlugin( &oPlugin );
		return false;
	}
	...
	{
		vstr::warnp() << "[VistaSystem]: Loading of plugin for driver ["
			<< sDriverType << "] failed - plugin loaded, has no driver creation method"
			<< std::endl;
		VddUtil::DisposePlugin( &oPlugin );
		return false;
	}
