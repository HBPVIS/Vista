...
public:
	...
	virtual bool AttachSequence(VistaWindow *pWindow)
	{
		...
		{
			// ugly workaround
			int windowId = static_cast<int>(reinterpret_cast<long long>(pWindow->GetWindowId()));
			// ok, register with the window is in the statics
			// section
			S_mapKeyboardMap.RegisterKeyboardWithWindow(windowId, m_pKeyboardDriver);
	...
	virtual bool DetachSequence(VistaWindow *pWindow)
	{
		...
		if(cit != m_mapWindows.end())
		{
			...
			S_mapKeyboardMap.UnregisterKeyboardFromWindow(static_cast<int>(reinterpret_cast<long long>(pWindow->GetWindowId())), m_pKeyboardDriver);
			return true;
		}
		return false;
	}
