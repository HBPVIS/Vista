...
public:
	...
	virtual bool AttachSequence( VistaWindow *pWindow )
	{
		...
		if( pGlutWTA == NULL )
		{
			vstr::errf() << "[GlutMouseDriver]: Trying to use "
		...
		{
			vstr::errf() << "[GlutMouseDriver]: Trying to arrach "
		...
		// section
		S_mapMouseMap.RegisterMouseWithWindow( iWindowID, m_pMouseDriver );
		...
		m_mapSensor[iWindowID] = _sSensorState(nIdx);
	...
	virtual bool DetachSequence(VistaWindow *pWindow)
	{
		...
		if(cit != m_mapWindows.end())
		{
			...
			int iWindowID = pGlutWTA->GetWindowId( pWindow );
			S_mapMouseMap.UnregisterMouseFromWindow( iWindowID, m_pMouseDriver );
			return true;
		}
		return false;
	}
