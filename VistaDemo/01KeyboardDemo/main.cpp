 ...
 *    Note that this is the deisred way of implementing actions, as
...
{
public:
	DirectKeyInput(VistaKeyboardSystemControl *pCtrl, 
		            VistaTextNode *pTextNode,
					VistaTextNode *pModNode,
					VistaSwitchNode *pSwitch)
...
private:
	...
	VistaSwitchNode *m_pSwitchNode; // just for this demo, make a switch node to switch between states in the view
};
...
{
public:
	ActivationCallback( VistaTextNode *pTextNode,
		const std::string &sTextToSet )
...
{
public:
	SwitchAction( VistaSwitchNode *pSwitch,
			       DirectKeyInput *pInput,
...
int main( int argc, char **argv )
{
	try
	{
		...
		if( oVistaSystem.Init( argc,argv ) )
		{
			...
			pText->GetTextImp()->SetFontSize(0.2f); // make it a bit smaller, so we can read it (hopefully)
			...
			pCtrl->BindAction( 'l', new ActivationCallback( pText, "l as in leave me" ), "set text to leave me" );
			pCtrl->BindAction( 'n', new ActivationCallback( pText, "n as in nothing" ), "set text to nothing" );
			...
			pCtrl->BindAction( VISTA_KEY_ESC,
				new SwitchAction( pSwitch, 
				new DirectKeyInput(pCtrl, pText, pModNode, pSwitch), pCtrl ), "switch direct input" );
