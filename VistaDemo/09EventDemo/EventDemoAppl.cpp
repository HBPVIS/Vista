		...
		m_pEventHandler( NULL )
{
	...
	m_vistaSystem.SetIniSearchPaths( liSearchPath );
	m_vistaSystem.IntroMsg ();
	bool state = m_vistaSystem.Init (argc, argv);
	CreateScene();

	// Get the EventManager from the system
	m_pEventManager = m_vistaSystem.GetEventManager();
	...
	VistaKeyboardSystemControl *pCtrl = m_vistaSystem.GetKeyboardSystemControl();
...
{
	VistaGraphicsManager* pGrMng = m_vistaSystem.GetGraphicsManager();
	if (pGrMng != NULL)
    {
