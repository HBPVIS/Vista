...
{
public:
	CameraSweep(VistaVirtualPlatform * pPlatform, float angle =Vista::Pi / 4.0f, float duration = 3.0f)
		: m_pVp(pPlatform)
		, axis(angle)
...
int main(int argc, char **argv)
{
	try
	{
		...
		if(pVistaSystem->Init(argc, argv))
		{
			...
			// self descriptive.
			pCtrl->BindAction( 't', new ToggleOverlay( oTextOverlay3D, pDispMgr),
				"Toggle 3D text overlay");

			pCtrl->BindAction( 'i', new ToggleOverlay( &oYawIndicator, pDispMgr),
