...
Alice::Alice(VistaKeyboardSystemControl *pCtrl, const std::string & sIniPath)
{
  ...
  else
  {
	...
	pCtrl->BindAction( 's', new AliceInitiateAction(m_pChannel,AliceInitiateAction::CMD_ZOOM_OUT),"ZOOM OUT");
	pCtrl->BindAction( 'a', new AliceInitiateAction(m_pChannel,AliceInitiateAction::CMD_TURN_LEFT),"TURN LEFT");
	pCtrl->BindAction( 'd', new AliceInitiateAction(m_pChannel,AliceInitiateAction::CMD_TURN_RIGHT),"TURN RIGHT");

	pCtrl->BindAction( VISTA_KEY_UPARROW, new AliceInitiateAction(m_pChannel,AliceInitiateAction::CMD_MOVE_UP),"MOVE UP");
	pCtrl->BindAction( VISTA_KEY_DOWNARROW, new AliceInitiateAction(m_pChannel,AliceInitiateAction::CMD_MOVE_DOWN),"MOVE DOWN");
	pCtrl->BindAction( VISTA_KEY_RIGHTARROW, new AliceInitiateAction(m_pChannel,AliceInitiateAction::CMD_MOVE_RIGHT),"MOVE RIGHT");
	pCtrl->BindAction( VISTA_KEY_LEFTARROW, new AliceInitiateAction(m_pChannel,AliceInitiateAction::CMD_MOVE_LEFT),"MOVE LEFT");
...
//This gets executed each time Alice transmits a message to Bob. He will answer and Alice will print!
//More in detail: This gets executed each time we press a key ;)
