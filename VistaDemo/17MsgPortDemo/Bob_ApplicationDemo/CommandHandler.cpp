...
CommandHandler::CommandHandler(VistaEventManager *pEvMgr, Bob *pBob)
	:	m_pEvMgr(pEvMgr), m_pBob(pBob)
{
	pEvMgr->AddEventHandler(this, VistaCommandEvent::GetTypeId(),
		VistaCommandEvent::VEIDC_CMD);
...
// Here we go. If this is called, the EventHandler has triggered because Alice sent an event/command.
void CommandHandler::HandleEvent(VistaEvent *pEvent)
{
	...
	default:
		std::cout << "Received an unknown Event: " << pCmdEv->GetMethodToken()
			<< std::endl;
