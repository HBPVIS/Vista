...
{
public:
	TranslateAction( VistaVirtualPlatform* pPlatform,
...
{
public:
	RotateAction( VistaVirtualPlatform *pPlatform,
			 float nDeg )
	...
	bool Do()
	{
		...
		VistaQuaternion qAxisAngle( VistaAxisAndAngle( VistaVector3D(0,1,0), Vista::DegToRad(m_fRot) ) );
...
int main(int argc, char **argv)
{
	try
	{
		...
		if(pVistaSystem->Init( argc, argv ))
		{
			...
			pCtrl->BindAction( VISTA_KEY_UPARROW, new TranslateAction( pVirtualPlatform,   VistaVector3D( 0, 0.25f, 0 ) ),
				"translate to y");
			pCtrl->BindAction( VISTA_KEY_DOWNARROW, new TranslateAction( pVirtualPlatform, VistaVector3D( 0, -0.25f, 0 ) ),
				"translate to -y");
