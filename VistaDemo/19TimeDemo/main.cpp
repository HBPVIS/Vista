...
void ShowTimeUtils()
{
	...
	VistaTimeUtils::ConvertToDate(nDate1, nMilliseconds, nSeconds, nMinute, nHour, nDay, nMonth, nYear );
	
	// we output the time, using its different components. We also use the vstr::formattime stream modifier,
	// which outputs the time with a higher precision (default: up to microseconds)
	vstr::outi() << "Date1 = " << vstr::formattime( nDate1 ) << " represents date "
				<< nYear << "-" << nMonth << "-" << nDay
				<< " at " << nHour << ":" << nMinute << ":" << nSeconds << "." << nMilliseconds << std::endl;
	VistaTimeUtils::ConvertToDate(nDate2, nMilliseconds, nSeconds, nMinute, nHour, nDay, nMonth, nYear );
	vstr::outi() << "Date2 = " << vstr::formattime( nDate2 ) << " represents date "
				<< nYear << "-" << nMonth << "-" << nDay
				<< " at " << nHour << ":" << nMinute << ":" << nSeconds << "." << nMilliseconds << std::endl;
	
	// this doesn't look too nice, because no leading zero are added as padding. We can use formatted strings
	// using the strftime syntax instrad
	vstr::outi() << "nDate2 = " << vstr::formattime( nDate2 ) << " represents date "
		<< VistaTimeUtils::ConvertToFormattedTimeString( nDate2, "%A, %d of %B %Y, %H:%M:%S" ) << std::endl;
	...
	vstr::outi() << "ConvertToDayTime( " << vstr::formattime( nDate2 ) << " ) = "
				<< vstr::formattime( VistaTimeUtils::ConvertToDayTime( nDate2 ) ) << std::endl;


	// VistaTimeUtils::Sleep allows to suspend the current thread for the requested amount of milliseconds
...
public:
	...
	{
		// to not spam the console with too much output, we only print the event timing info every 100'th frame
		...
		{
			std::cout << "\n---------------------------------------------------\n" 
						"Frame " << std::setw( 8 ) << nFrameCount << " with FrameClock "
						<< std::setw( 20 ) << vstr::formattime( m_pVistaSystem->GetFrameClock() ) << std::endl;
		}
		...
		std::cout << "SystemEvent [" << std::setw( 30 ) << VistaSystemEvent::GetIdString( pSysEvent->GetId() )
			<< "] at " << std::setw( 20 ) << vstr::formattime( pSysEvent->GetTime() ) << std::endl;
...
void ShowVistaTimer()
{
	...
	VistaTimer oTimer;
	// Alternatively, a standard timer can be used
	...
	VistaType::systemtime nAbsoluteTime = oTimer.GetSystemTime();
	...
	VistaType::microtime nRelativeTime = oTimer.GetMicroTime();
	// microstamp is an arbitrary, monotonously rising counter
	VistaType::microstamp nStamp = oTimer.GetMicroStamp();
	...
	std::cout << "VistaTimer::GetSystemTime()  accuracy is about " << vstr::formattime( 1e9 * nSystemTimeDelta, 3 ) << " nanoseconds" << std::endl;
	...
	std::cout << "VistaTimer::GetMicroTime()   accuracy is about " << vstr::formattime( 1e9 * nMicroTimeDelta, 3 ) << " nanoseconds" << std::endl;
	...
	std::cout << "Calculating the value " << nRes << " took " << vstr::formattime( 1e3 * nDuration, 3 ) << " milliseconds" << std::endl;

	// for periodic operations, one can use the VistaAverageTimers, of which there are differnet variants
	// the AverageTimer takes the average over all samples
	...
	std::cout << "When calculating the value " << nRes << ", iterations required on average:"
		<< "\nNormal Average         " << std::setw( 20 ) << vstr::formattime( 1e3 * oAverage.GetAverageTime(), 3 ) << " milliseconds"
		<< "\n12-Window Average      " << std::setw( 20 ) << vstr::formattime( 1e3 * oWindowAverage.GetAverageTime(), 3 ) << " milliseconds"
		<< "\nFast Weighted Average  " << std::setw( 20 ) << vstr::formattime( 1e3 * oFastAverage.GetAverageTime(), 3 ) << " milliseconds"
		<< "\nSlow Weigted Average   " << std::setw( 20 ) << vstr::formattime( 1e3 * oSlowAverage.GetAverageTime(), 3 ) << " milliseconds"
		<< std::endl;

	// With all these, again keep in mind that timing in cluster-mode can be tricky.
	// Using the VistaTimer for debugging or performance measurements is fine. However, its time values
	// should not be used to influence application logic, otherwise cluster asynchonization is looming
...
void SetupTimeEventHandlers( VistaSystem* pSystem )
{
	...
	// tick duration is exceeded. We first create the tick timer (note: we'll not store it, so its a memleak - don't do that in real apps)
	...
	// The TickTimer checks the interval once every frame, using the frameclock, during the POSTAPPLICATIONLOOP event.
	// Thus, it can only work with the granularity of one frame
	// To receive the tick time events, we create and register an event observer
	// Pardon the function-local class - its just to keep the flow in this file ;)
	class TickTimerHandler : public VistaEventObserver
	{
	public:
		virtual void Notify( const VistaEvent *pEvent )
		{
			const VistaTickTimerEvent* pTickEvent = Vista::assert_cast< const VistaTickTimerEvent* >( pEvent );
			...
			std::cout << "Received TickTimerEvent at time " 
						<< std::setw( 20 ) << vstr::formattime( pEvent->GetTime() )
						<< ", requested tick time: " << std::setw( 10 )
						<< pTickEvent->GetSourceTickTimer()->GetTickTime() << sGranularityPostfix
						<< ", actual time: " << std::setw( 10 )
						<< pTickEvent->GetSourceTickTimer()->GetRealIntervalLength() << sGranularityPostfix << std::endl;
		}
	};
	...
	VistaTimeoutRouter* pTimeoutRouter = new VistaTimeoutRouter( pEventManager, pSystem->GetClusterMode() );
	// we write a class for the callbacks
	class TimeoutRouterCallback : public VistaTimeoutRouter::CTimedCallback
	{
	public:
		TimeoutRouterCallback( const std::string& sName ) : m_sName( sName ) {}
		virtual bool DoCallback( double dTimeStamp, double dDesiredLength, double dIntervalLength )
		{
			std::cout << "TimeutRouterHandler [" << std::setw( 22 ) <<  m_sName
						<< "] called at time " << std::setw( 20 ) << vstr::formattime( dTimeStamp )
						<< ", requested tick time: " << std::setw( 10 )
						<< dDesiredLength << "ms"
						<< ", actual time: " << std::setw( 10 )
						<< dIntervalLength << "ms" << std::endl;
			return true;
		}
