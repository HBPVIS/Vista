...
const int s_nCompotationExternalMessageTag = 666;
...
void ClusterModeDemo::SetupExternalMessageEvent()
{
	...
	m_pMessage->SetMsgTicket( s_nCompotationExternalMessageTag );
...
void ClusterModeDemo::HandleExternalMessageEvent( VistaEvent* pEvent )
{
	...
	if( pMsgEvent->GetThisMsg()->GetMsgTicket() == s_nCompotationExternalMessageTag )
	{
		// yay, data!		
		std::string sData;
