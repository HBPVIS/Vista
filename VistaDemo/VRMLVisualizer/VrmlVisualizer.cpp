		...
		m_vecArgs( std::vector<std::string>() )
{
	...
	if( !m_vrmlProps.HasProperty( "filename" ) && 
		!m_vrmlProps.HasProperty( "dirname" ) )
	{
		PrintUsage();
		VISTA_THROW( "[VRMLVisualizer] No file or directory given.", 1 );
	}

	m_vistaSystem.GetEventManager()->AddEventHandler( &m_graphicsEventHandler,
	...
	pKeyCtrl->BindAction( 'd', new EnDisableCallback( m_loadedNodes ), "enable / disable all loaded nodes." );
	pKeyCtrl->BindAction( ' ', new ToggleAnimationCallback( m_vrmlProps ), "enable / disable animation." );
...
bool VrmlVisualizer::LoadData()
{
	...
	if( m_vrmlProps.HasProperty( "dirname" ) )
	{
		...
		for( VistaFileSystemDirectory::const_iterator it = dir.begin();
				it != dir.end();
				++it)
		{
			if( (*it)->IsFile() )
...
VistaTransformNode * VrmlVisualizer::FirstNodeAsTransformNode( IVistaNode *pNode )
{
	...
	else
	{
		...
		pLocalRoot = m_vistaSystem.GetGraphicsManager()->GetSceneGraph()->
			NewTransformNode( dynamic_cast<VistaGroupNode*>(pParent) );
...
void VrmlVisualizer::ParseParameters()
{
	...
	{
		if( std::string("--filename").compare( m_vecArgs[i] ) == 0 || std::string("-f").compare( m_vecArgs[i] ) == 0 )
		{
			std::cout << "f" << std::endl;
